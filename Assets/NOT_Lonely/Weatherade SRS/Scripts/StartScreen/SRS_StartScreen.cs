            ...
            if(menuButton == MenuButton.URP)
            {
                if (SRS_VersionCheck.IsUsingURP())
                {
                    ...
                    if (GUILayout.Button("Import URP", GUILayout.MaxWidth(100)))
                    {
                        ...
                        //extract addition package to support Unity 6.1+ URP
                        AssetDatabase.ImportPackage("Assets/NOT_Lonely/Weatherade SRS/URP Support/WeatheradeSRS_URP_17_1.unitypackage", false);
#endif
                    }
                    GUILayout.FlexibleSpace();
                    GUILayout.EndHorizontal();
