namespace RaycastPro
{
    ...
    public abstract class BaseRaySensor<R, E, P> : RaycastCore where P : BasePlanar
    {
        ...
        protected void PlanarField(SerializedObject _so)
        {
            ...
            {
                BeginVerticalBox();
                RCProEditor.DrawSerializedList(_so.FindProperty(nameof(planers)));
                EndVertical();
            }
        }
...
// ADD
            if (!liner && GUILayout.Button(CAdd, GUILayout.Width(50f)))
            {
                ...
                {
                    liner = lineRenderer;
                    prop.objectReferenceValue = liner;
                }
                else
                {
                    var newVal = (_so.targetObject as MonoBehaviour)
                        ?.gameObject.AddComponent<LineRenderer>();

                    Undo.RegisterCreatedObjectUndo(newVal, "Create Liner");
                    prop.objectReferenceValue = newVal;
                    liner = (LineRenderer)prop.objectReferenceValue;
                }

                StaticUtilities.LinerProfile.Apply(liner, RCProPanel.linerMaxWidth);
                UpdateLiner();
            }

// REMOVE
            if (liner && GUILayout.Button("Remove", GUILayout.Width(60f)))
            {
                Undo.RecordObject(_so.targetObject, "Remove Liner");

                // حذف کامپوننت با Undo
                Undo.DestroyObjectImmediate(liner);
        ...
        private static Material _defaultLinerMat;

        private static Material GetDefaultLinerMaterial()
        {
            if (_defaultLinerMat) return _defaultLinerMat;

            // بهترین انتخاب برای لاین‌های هنری
            var shader = Shader.Find("Universal Render Pipeline/Unlit");
            if (!shader)
                shader = Shader.Find("Unlit/Color");

            _defaultLinerMat = new Material(shader)
            {
                name = "RCPro_Liner_Default"
            };

            _defaultLinerMat.SetColor("_BaseColor", Color.white);
            _defaultLinerMat.EnableKeyword("_SURFACE_TYPE_TRANSPARENT");
            _defaultLinerMat.renderQueue = (int)UnityEngine.Rendering.RenderQueue.Transparent;
