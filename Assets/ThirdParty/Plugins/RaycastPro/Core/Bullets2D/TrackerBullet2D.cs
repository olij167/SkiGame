...
#if UNITY_EDITOR
        internal override string Info => "A guided projectile that actively seeks a specified target. It supports two distinct tracking behaviors: 'PositionLerp' for a direct, linear approach, and a more physically-based 'RotationLerp' where the projectile realistically rotates towards the target and propels itself forward with a configurable force. The 'RotationLerp' mode is governed by parameters like turn sharpness and drag, allowing for the creation of natural, curved pursuit trajectories. The projectile's lifecycle ends when it reaches a proximity threshold to its target, runs out of force, or its lifetime expires."+ HAccurate + HDependent;
