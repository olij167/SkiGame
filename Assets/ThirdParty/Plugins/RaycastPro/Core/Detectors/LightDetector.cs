    ...
    public sealed class LightDetector : Detector, IPulse
    {
...
#if UNITY_EDITOR
        internal override string Info => "A light occlusion sensor that measures the total illumination at its position. It casts rays to check for line-of-sight obstruction from specified Directional and Point lights, calculating a final score based on the aggregated intensity of all unblocked sources."
                                         + HDependent +HIPulse;
            ...
            bool hasInfo = true)
        {
            if (hasMain)
            {
                ...
                if (!lightFinder)
                {
                    RCProEditor.DrawSerializedList(_so.FindProperty(nameof(sources)));
                }
