...
#if UNITY_EDITOR
        internal override string Info => "A versatile multi-hit cast sensor that detects all collisions along its path. It dynamically supports various casting types, including Ray, Sphere, Capsule, and Box, with optional non-allocated memory usage for performance optimization."
                                         + HDirectional+ HIRadius + HIPulse + HRDetector + HINonAllocator;
