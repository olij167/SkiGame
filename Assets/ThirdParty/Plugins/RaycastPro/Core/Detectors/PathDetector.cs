...
#if UNITY_EDITOR
        internal override string Info => "A path aggregator that operates on a source ray, detecting and collecting all colliders that intersect the source's path geometry. The collection sweep respects the source ray's radius."
                                         + HAccurate + HIPulse + HPathRay + HRDetector + HIRadius + HDependent;
