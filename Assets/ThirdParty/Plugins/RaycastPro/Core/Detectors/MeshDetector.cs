...
#if UNITY_EDITOR
        internal override string Info => "A mesh-based volumetric sensor that uses a custom MeshCollider to define an arbitrary detection volume. It accurately identifies line-of-sight targets contained within the mesh's geometry using a precise raycast containment test."
                                         + HDependent + HCDetector + HLOS_Solver + HRotatable + HScalable + HINonAllocator;
