...
// اگر خارج از آگاهی کامل بود
                if (tempDis > fullAwareness * fullAwareness)
                {
                    if (tempDis > radius * radius || tempDis < minRadius * minRadius)
                        continue;

                    // زاویه X: افقی (روی صفحه عمود بر up)
                    var flatVec = Vector3.ProjectOnPlane(_tVec, up).normalized;
                    var dotX = Vector3.Dot(flatVec, forward); // cos(angleX)
                    if (dotX < Mathf.Cos(angleX * 0.5f * Mathf.Deg2Rad))
                        continue;

                    // زاویه Y: عمودی (بین بردار هدف و صفحه افقی)
                    var dotY = Vector3.Dot(_tVec.normalized, flatVec); // cos(angleY)
                    if (dotY < Mathf.Cos(angleY * 0.5f * Mathf.Deg2Rad))
                        continue;
                }
...
#if UNITY_EDITOR
        internal override string Info => "A sophisticated perception sensor that simulates a realistic field of view. It provides 360-degree awareness at close range, while filtering distant objects based on a configurable viewing angle (FoV) and direct line-of-sight (LOS) visibility." + HCDetector + HLOS_Solver + HIPulse + HIRadius + HINonAllocator;
