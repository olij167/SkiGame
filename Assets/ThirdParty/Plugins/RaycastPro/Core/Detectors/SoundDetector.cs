    ...
    public sealed class SoundDetector : Detector, IPulse
    {
...
#if UNITY_EDITOR
        internal override string Info => "An advanced acoustic sensor that simulates realistic sound propagation. It calculates the perceived loudness of AudioSources by modeling attenuation from both distance and the thickness of obstructing materials." +HAccurate+HIPulse+HDependent;
            ...
            bool hasInfo = true)
        {
            if (hasMain)
            {
                ...
                {
                    BeginVerticalBox();
                    RCProEditor.DrawSerializedList(prop);
                    EndVertical();
                }
