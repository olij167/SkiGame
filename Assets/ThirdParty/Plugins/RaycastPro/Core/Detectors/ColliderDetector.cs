...
namespace RaycastPro.Detectors
{
    ...
    public abstract class ColliderDetector : Detector
    {
        ...
        public bool collectBlockedLOSHits;
        [FormerlySerializedAs("collectLosPoints")] [Tooltip("Detector collect current position of solver Points \"DirectLOSPoints\" Dictionary. Key: Collider, Value: Vector3")]
        public bool collectLOSPoints;
        ...
        protected readonly Dictionary<Collider, Vector3> directLOSPoints = new Dictionary<Collider, Vector3>();
        public Dictionary<Collider, Vector3> DirectLOSPoints => directLOSPoints;
        #region Methods
        ...
        {
            DetectedColliders.Clear();
            if (collectBlockedLOSHits) detectedLOSHits.Clear();
            if (collectLOSPoints) directLOSPoints.Clear();
        }
        /// <summary>
        ...
        protected bool LOSPass(Vector3 point, Collider c)
        {
            ...
            {
                if (collectBlockedLOSHits) detectedLOSHits.Add(c, blockHit);
#if UNITY_EDITOR
        ...
        protected void SetupGates(Collider c, Vector3 point, bool blocked, RaycastHit blockHit)
        {
            ...
            GizmoGate += () =>
            {
                if (blocked)
                {
                    DrawBlockLine(transform.position, point, blockHit);
        ...
        protected void SolverField(SerializedObject _so)
        {
            BaseSolverField(_so, () =>
            {
                ...
                EditorGUILayout.PropertyField(_so.FindProperty(nameof(collectBlockedLOSHits)));
                EditorGUILayout.PropertyField(_so.FindProperty(nameof(collectLOSPoints)));
                if (IsPivotSolver)
                {
