...
#if UNITY_EDITOR
        internal override string Info => "A beam-splitting planar modifier for the 'Planar Sensitive' ray system. Upon impact, it acts as a splitter, consuming the incoming ray and generating a fan of multiple new rays from the point of contact. The number of outgoing rays and their angular spread are configurable, allowing for a range of effects from a simple path fork to a wide, shotgun-like dispersion. This is ideal for designing complex puzzles where a single beam must activate several targets simultaneously."+HDependent+HExperimental;
