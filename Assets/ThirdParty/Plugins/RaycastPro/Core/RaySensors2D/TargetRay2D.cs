...
#if UNITY_EDITOR
        internal override string Info => "A versatile 2D directional sensor that dynamically performs either a Linecast or a CircleCast based on the configured radius. When the radius is zero, it casts a single line to detect collisions. When a positive radius is provided, it performs a CircleCast, effectively sweeping a circular volume along its path to identify the first overlapping collider. This dual-mode capability makes it suitable for both precise point-to-point detection and broader volumetric checks in 2D environments."+HAccurate + HIRadius + HDependent;
