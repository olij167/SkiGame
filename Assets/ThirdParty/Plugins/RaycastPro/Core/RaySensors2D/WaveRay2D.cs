...
#if UNITY_EDITOR
        internal override string Info => "A dynamic, procedural path sensor that generates an animated, wave-like trajectory. The path is based on a sinusoidal function whose phase is continuously updated over time according to the 'waveSpeed', creating the effect of a traveling wave. After constructing the animated wave geometry, the component performs a cast along its entire length to detect collisions. This makes it ideal for simulating effects like sonar pings, energy pulses, or creating serpentine, oscillating detection patterns."
