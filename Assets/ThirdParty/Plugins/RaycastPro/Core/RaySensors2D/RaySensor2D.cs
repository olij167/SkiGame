namespace RaycastPro.RaySensors2D
{
    ...
    public abstract class RaySensor2D : BaseRaySensor<RaycastHit2D, RaycastEvent2D, Planar2D>
    {
        ...
        private void UpdateClamped()
        {
            Vector3 start = Vector3.Lerp(_base, _tip, linerBasePosition);
            Vector3 end   = Vector3.Lerp(_base, _tip, linerEndPosition);
            ...
            Vector3 hitPoint = linerFixCut
                ? GetPointOnLine(_base, _tip, HitPointZ)
                : HitPointZ;

            float ratio = hit.transform
                ? (hitPoint - _base).magnitude / RayLength
                : 1f;

            if (ratio < linerBasePosition)
            {
            ...
            liner.SetPosition(0, start);
            liner.SetPosition(1, ratio < linerEndPosition ? hitPoint : end);
        }
        private void UpdateUnclamped()
        {
            ...
            liner.SetPosition(
                1,
                LinerFixCut
                    ? _base + Vector3.Project(target - _base, Direction)
                    : target
            );
        }
