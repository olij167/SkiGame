...
#if UNITY_EDITOR
        internal override string Info => "A 2D sensor that casts a ray from its origin towards the current mouse cursor position. It is compatible with both Unity's legacy Input Manager and the modern Input System package. The sensor intelligently switches between a precise Linecast and a more robust, volumetric CircleCast based on the configured radius. An optional vertical offset can be applied to the mouse target point, and the maximum cast distance can be explicitly defined."
                                         +HAccurate+HIRadius+HDependent;
