...
#if UNITY_EDITOR
        internal override string Info => "A composite path sensor that processes and combines an array of source RaySensors, operating in two distinct modes. In 'PathCast' mode, it first stitches the source sensors into a single, continuous trajectory and then performs a new cast along this composite path to find a final hit. Alternatively, 'SequenceCast' mode acts as a prioritizer, iterating through the source sensors in order and returning the very first successful hit it encounters. This component enables the creation of highly complex and conditional detection logic by chaining simpler sensors together."+ HDependent + HPathRay;
