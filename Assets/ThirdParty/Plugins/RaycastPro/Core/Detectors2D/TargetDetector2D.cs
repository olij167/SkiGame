...
#if UNITY_EDITOR
        internal override string Info => "A specialized 2D Line-of-Sight (LOS) verifier designed to monitor a predefined list of specific targets. For each transform in its target array, the component performs a direct LOS check from its own position, which can be either a precise Linecast or a more robust, volumetric CircleCast. It maintains a dynamic list of all currently visible (unobstructed) targets and features a comprehensive event system that fires when a target first comes into view and again when the line of sight is lost. This makes it an ideal tool for scenarios where a fixed set of points or objects needs to be consistently monitored, such as a security camera system checking its key waypoints, an AI guard verifying its patrol points, or a turret validating its pre-assigned targets before engaging."
                                         +HAccurate+HIRadius+HIPulse;
