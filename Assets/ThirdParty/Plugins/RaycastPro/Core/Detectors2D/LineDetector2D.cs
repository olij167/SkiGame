    ...
    public sealed class LineDetector2D : ColliderDetector2D, IRadius, IPulse
    {
        ...
        protected override void OnCast()
        {
            ...
            if (limited)
            {
                ...
                {
                    case RayType.Ray:
                        hitsCount = Physics2D.RaycastNonAlloc(transform.position, LocalDirection.normalized,
                            nonAllocatedHits,
                    ...
                    case RayType.Pipe:
                        if (height > 0)
                        {
                            ...
                            hitsCount = Physics2D.CapsuleCastNonAlloc(transform.position,
                                new Vector2(Mathf.Max(radius * 2, 0.001f), height + radius * 2),
                                CapsuleDirection2D.Vertical, angle, Direction, nonAllocatedHits, direction.magnitude,
                                detectLayer.value, MinDepth, MaxDepth);
                        }
                        else
                        {
                            hitsCount = Physics2D.CircleCastNonAlloc(transform.position, radius, Direction,
                                nonAllocatedHits, direction.magnitude, detectLayer.value, MinDepth, MaxDepth);
                        }

                        break;
                    case RayType.Box:
                        hitsCount = Physics2D.BoxCastNonAlloc(transform.position, extents / 2,
                            Vector2.Angle(Direction, Vector2.right), LocalDirection, nonAllocatedHits,
                            direction.magnitude, detectLayer.value, MinDepth, MaxDepth);
                        break;
                }
            ...
            else
            {
                ...
                switch (rayType)
                {
                    ...
                    case RayType.Pipe:
                        ...
                        {
                            angle = local ? -Vector2.Angle(transform.right.To2D(), Vector2.right) : 0;
                            DetectedHits.AddRange(Physics2D.CapsuleCastAll(transform.position,
                                new Vector2(Mathf.Max(radius * 2, 0.001f), height + radius * 2),
                                CapsuleDirection2D.Vertical, angle, Direction, direction.magnitude, detectLayer.value,
                                MinDepth, MaxDepth));
                        }
                        else
                        {
                            DetectedHits.AddRange(Physics2D.CircleCastAll(transform.position, radius, Direction,
                                direction.magnitude, detectLayer.value, MinDepth, MaxDepth));
                        }
        ...
        internal override string Info =>
            "A versatile 2D multi-hit sensor designed to detect and collect all colliders along a cast path, not just the first. It supports a variety of dynamic casting shapes, including Ray, Box, and a smart 'Pipe' mode that intuitively switches between CircleCast and CapsuleCast. A critical feature is its optional high-performance mode that utilizes non-allocating physics queries to prevent runtime garbage collection, making it ideal for performance-sensitive applications. The component is further enhanced with built-in tag filtering and a comprehensive event system for managing new and lost targets." +
            HDirectional + HRDetector + HIRadius + HIPulse + HINonAllocator;
