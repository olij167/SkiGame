...
#if UNITY_EDITOR
        internal override string Info => "A highly efficient 2D sensor that leverages a `PolygonCollider2D` to define a completely custom and arbitrary detection shape. Instead of performing complex geometric calculations, it uses the optimized, built-in `PolygonCollider2D.Overlap` method, providing precise results filtered by a `ContactFilter2D`. This component functions as a pure volumetric trigger, identifying any collider that physically overlaps its shape without performing additional line-of-sight checks for occlusion. It includes a performance-boosting, non-allocating mode, making it the ideal tool for creating complex and irregular-shaped trigger zones, such as safe areas, liquid volumes, or unique fields of effect."+ HCDetector + HLOS_Solver + HINonAllocator + HDependent;
