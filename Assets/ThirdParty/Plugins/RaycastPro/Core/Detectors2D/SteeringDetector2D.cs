...
#if UNITY_EDITOR
        internal override string Info => "An advanced 2D steering and obstacle avoidance agent that computes an optimal, smooth navigation vector. If the direct path to the destination is blocked, it resorts to a sophisticated, multi-layered solving strategy: it probabilistically samples the local environment with random rays, validates potential escape routes with a secondary 'spider' cast, and leverages a short-term positional memory ('mark solver') to navigate complex blockages and prevent getting stuck. The final output is a smoothed steering direction, ideal for guiding dynamic characters and AI in complex 2D worlds."
                                         + HAccurate + HRDetector + HIRadius;
