...
#if UNITY_EDITOR
        internal override string Info => "A 2D path aggregator that operates on a source ray, detecting and collecting all colliders that intersect the source's path geometry. The collection sweep intelligently adapts to the source ray's properties, performing a volumetric (circular) path cast if a radius is defined, or a linear cast otherwise. This makes it ideal for gathering a complete list of all objects affected by a complex trajectory, such as a ricocheting laser or a curved projectile path."
                                         + HAccurate + HPathRay + HIPulse + HRDetector + HDependent;
