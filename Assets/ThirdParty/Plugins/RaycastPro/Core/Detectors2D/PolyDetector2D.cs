...
#if UNITY_EDITOR
        internal override string Info => "A versatile 2D sensor that defines a regular polygonal (n-gon) detection area. It employs an efficient, two-phase detection process: a broad-phase OverlapCircle quickly gathers potential targets, which are then precisely verified by a narrow-phase point-in-polygon test. This ensures that only colliders truly inside the custom shape are considered. The sensor further refines its results with a line-of-sight check to filter out occluded objects. It includes a high-performance, non-allocating mode and is ideal for creating crisp, custom-shaped trigger zones with a definable number of edges, offering more precision than a standard circle."+ HCDetector + HLOS_Solver + HINonAllocator;
