...
#if UNITY_EDITOR
        internal override string Info => "A 2D radar-style scanner that simulates a rotating line of sight by sweeping a raycast in a 360-degree arc. Its key feature is a caching system that causes detected targets to persist for a configurable duration, simulating the 'blip' of a target on a radar screen that fades over time if not re-acquired. For accuracy, it performs a secondary line-of-sight check on each potential target to filter out occluded objects. This makes it perfect for creating minimap systems, enemy AI perception that tracks recently seen players, or any scenario requiring a continuous, sweeping detection field with memory."+ HCDetector + HIRadius + HRotatable;
