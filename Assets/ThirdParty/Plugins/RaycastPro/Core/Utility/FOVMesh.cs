namespace RaycastPro.Sensor
{
    ...
    {
        [Tooltip("Length of the view cone.")] public float Length = 5f;

        [Tooltip("Base size of the cone.")] public float BaseSize = 0.5f;

        [Range(1f, 180f)] public float HorizontalFOV = 90f;

        [Range(1f, 180f)] public float VerticalFOV = 90f;

        [Range(3, 32)] public int SegmentCount = 8;
...
// عرض مربع ابتدایی
        ...
        void GenerateFOVMeshCollider()
        {
            ...
            {
                Debug.LogWarning(
                    $"FOV Mesh has {triangleCount} triangles, which exceeds Unity's convex mesh limit. Reducing SegmentCount.");
            ...
            if (forceNonConvex)
                Debug.LogWarning(
                    $"FOV Mesh has too many triangles ({triangles.Length / 3}). Using non-convex collider.");
...
#if UNITY_EDITOR
        internal override string Info => "A utility component that procedurally generates a 3D mesh shaped like a view frustum. Its primary purpose is to provide a custom, configurable volume for a MeshCollider, enabling the creation of realistic, cone-shaped trigger zones for perception sensors (e.g., a character's field of view). It is designed as a companion tool for mesh-based sensors like the Mesh Detector, automatically handling mesh generation and respecting Unity's convex collider constraints."
                                         + HUtility +
            HDependent;
