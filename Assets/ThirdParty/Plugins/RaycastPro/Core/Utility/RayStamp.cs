...
#if UNITY_EDITOR
        internal override string Info => "A versatile path marker component that procedurally places a 'stamp' Transform at a specific point along a source sensor's trajectory. It offers three distinct positioning modes: 'Clamp' for placing the marker at a relative percentage of the path's length, 'Distance' for an absolute distance from the start, and 'Offset' for a fixed distance from the end. The marker can intelligently calculate its position relative to either the full path length or the actual hit point of the sensor. It also features optional orientation syncing to align with the path's direction and can automatically hide when the source sensor detects no target."
                                         + HUtility + HDependent;
