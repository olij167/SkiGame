...
#if UNITY_EDITOR
        internal override string Info => "A highly modular composite path sensor that constructs a single, unified trajectory from an array of source RaySensors. It features a powerful 'daisy-chaining' mode ('sequenceOnTip') where each sensor in the sequence can dynamically start from the tip of the previous one, creating a continuous, snake-like path. It then operates in two distinct ways: 'PathCast' mode performs a single, final sweep along the entire combined geometry to find a collision, while 'SequenceCast' mode acts as a prioritizer, returning the very first hit found among the individual sensors in their array order. This enables the creation of incredibly complex, conditional, and procedural detection patterns that would be impossible with a single sensor." + HDependent + HPathRay;
