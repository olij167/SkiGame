namespace RaycastPro.RaySensors
{
    ...
    {
        [Tooltip(
            "This option enables the calculation of the series of rays in Path Ray. (If you only use path points in Path Detector or..., you can disable it.)")]
        ...
        /// <summary>
        /// Performs a collision check along the path between consecutive path points.
        /// Uses Linecast when <paramref name="radius"/> is zero, and SphereCast with a
        /// constant radius when greater than zero. Returns the index of the first path
        /// segment that detects a collision, or -1 if no hit occurs.
        /// </summary>
        /// <param name="radius">
        /// The fixed radius used for SphereCast along the entire path.
        /// If set to zero, a precise Linecast is performed instead.
        /// </param>
        /// <returns>
        /// The index of the path segment where the first collision is detected,
        /// or -1 if the path is completely clear.
        /// </returns>
        protected int PathCast(float radius = 0f)
        {
            ...
            else
            {
                for (var i = 0; i < PathPoints.Count - 1; i++)
                {
                    if (!Physics.Linecast(PathPoints[i], PathPoints[i + 1], out hit, detectLayer.value,
                            triggerInteraction)) continue;
        ...
        /// <summary>
        /// Performs an advanced collision check along the path with support for
        /// a variable radius. The cast radius is smoothly interpolated from
        /// <paramref name="startRadius"/> to <paramref name="radius"/> per path segment.
        /// If both radii are zero, the method falls back to a Linecast.
        /// Returns the index of the first segment that detects a collision, or -1 if no hit occurs.
        /// </summary>
        /// <param name="startRadius">
        /// The initial radius at the beginning of the path. Allows defining a thicker
        /// or thinner collision volume at the start.
        /// </param>
        /// <param name="radius">
        /// The final radius at the end of the path. When combined with startRadius,
        /// determines the interpolated SphereCast radius along the path.
        /// </param>
        /// <returns>
        /// The index of the path segment where the first collision is detected,
        /// or -1 if the entire path is unobstructed.
        /// </returns>
        protected int AdvancePathCast(float startRadius, float radius = 0f)
        {
            ...
            for (int i = 0; i < count - 1; i++)
            {
                ...
                if (useSphere)
                {
                    ...
                    hitDetected = Physics.SphereCast(from, lerpedRadius, dir, out hit, distance, detectLayer.value,
                        triggerInteraction);
                }
                else
                {
            ...
            if (linerClamped)
            {
                ...
                if (linerCutOnHit && hit.transform) // Cut and detect Hit
                {
                    ...
                    var hitPoint = linerFixCut
                        ? GetPointOnLine(PathPoints[detectIndex], PathPoints[detectIndex + 1], hit.point)
                        : HitPoint;
        ...
        protected void FullPathDraw(float radius = 0f, bool cap = false, bool dotted = false)
        {
            ...
            DrawPath(PathPoints, hit, radius, detectIndex: DetectIndex, drawSphere: false, dotted: dotted,
                coneCap: cap);
        }
        ...
        protected void AdvancePathDraw(float startRadius, float radius = 0f, bool cap = false, bool dotted = false)
        {
            ...
            DrawAdvancePath(PathPoints, hit, startRadius, radius, detectIndex: DetectIndex, drawSphere: false,
                dotted: dotted, coneCap: cap);
        }
