...
namespace CodeStage.Maintainer.Tools
{
	...
	internal static class CSEditorTools
	{
		...
		public static long GetCurrentAllocatedMemoryBytes()
		{
			return UnityEngine.Profiling.Profiler.GetTotalAllocatedMemoryLong();
		}

		public static void CapturePeakAllocatedMemory(ref long peakAllocatedMemoryBytes)
		{
			peakAllocatedMemoryBytes = Math.Max(peakAllocatedMemoryBytes, GetCurrentAllocatedMemoryBytes());
		}

		public static string FormatMaxReservedMemory(long initialAllocatedMemoryBytes, long peakAllocatedMemoryBytes)
		{
			var reservedMemoryBytes = Math.Max(0, peakAllocatedMemoryBytes - initialAllocatedMemoryBytes);
			var reservedMemoryMegabytes = reservedMemoryBytes / (1024d * 1024d);
			return string.Format(CultureInfo.InvariantCulture, "MEM: {0:0.00} MB", reservedMemoryMegabytes);
		}
