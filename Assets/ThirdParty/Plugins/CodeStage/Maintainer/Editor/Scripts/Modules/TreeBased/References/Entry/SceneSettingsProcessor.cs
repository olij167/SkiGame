	...
	internal static class SceneSettingsProcessor
	{
		...
		public static void Process(List<TreeConjunction> conjunctions)
		{
			...
			foreach (var conjunction in conjunctions)
			{
				...
				{
					var candidate = CSEntityIdTools.IdToObject(candidateInstanceId);
		...
		private static void CheckNavMesh(TreeConjunction conjunction, int candidateInstanceId)
		{
			...
			{
				if (CSEntityIdTools.GetObjectReferenceId(navMeshDataField) == candidateInstanceId)
				{
		...
		{
			if (Lightmapping.lightingDataAsset == null || CSEntityIdTools.GetId(Lightmapping.lightingDataAsset) != candidateInstanceId) return;
		...
		private static void CheckLightingSettingsAsset(TreeConjunction conjunction, int candidateInstanceId)
		{
			try
			{
				if (Lightmapping.lightingSettings == null ||
					CSEntityIdTools.GetId(Lightmapping.lightingSettings) != candidateInstanceId)
					return;
		...
		{
			if (RenderSettings.skybox == null || CSEntityIdTools.GetId(RenderSettings.skybox) != candidateInstanceId) return;
		...
		private static void CheckLightMapSettings(TreeConjunction conjunction, int candidateInstanceId)
		{
			...
			{
				if (CSEntityIdTools.GetObjectReferenceId(lightmapParametersField) == candidateInstanceId)
				{
					var entry = new ReferencingEntryData
					{
						location = Location.SceneLightingSettings,
		...
		private static void CheckLightingCubemap(TreeConjunction conjunction, int candidateInstanceId)
		{
			...
			if (reflection == null ||
				CSEntityIdTools.GetId(reflection) !=
		...
		{
			if (RenderSettings.sun == null || CSEntityIdTools.GetId(RenderSettings.sun) != candidateInstanceId) return;
		...
		private static void CheckRenderSettingsTexture(TreeConjunction conjunction, int candidateInstanceId)
		{
			...
			{
				if (CSEntityIdTools.GetObjectReferenceId(renderHaloField) == candidateInstanceId)
				{
					var entry = new ReferencingEntryData
					{
						location = Location.SceneLightingSettings,
						prefixLabel = ConstructOtherSettingsPath("Halo Texture")
					};
			...
			{
				if (CSEntityIdTools.GetObjectReferenceId(renderSpotField) == candidateInstanceId)
				{
					var entry = new ReferencingEntryData
					{
						location = Location.SceneLightingSettings,
						prefixLabel = ConstructOtherSettingsPath("Spot Cookie")
					};
		...
		private static string ConstructOtherSettingsPath(string settingName)
		{
