...
namespace CodeStage.Maintainer.References.Entry
{
	...
	internal static class ProjectEntryFinder
	{
		public static bool FillProjectScopeReferenceEntries(List<AssetConjunctions> conjunctionInfoList, ProcessObjectReferenceHandler processReferenceCallback)
		{
			...
			var hasSceneTargets = false;
			for (var i = 0; i < count; i++)
			{
				if (conjunctionInfoList[i].asset.Type != CSReflectionTools.sceneAssetType)
					continue;

				hasSceneTargets = true;
				break;
			}

			// Temporarily disable automatic lighting generation to prevent Unity from
			// rebaking lightmaps and reflection probes when scenes are opened,
			// which would dirty scenes and generate changed files on disk.
#if UNITY_6000_0_OR_NEWER
			Lightmapping.BakeOnSceneLoadMode savedBakeOnSceneLoad = default;
#else
			Lightmapping.GIWorkflowMode savedGIWorkflowMode = default;
#endif
			if (hasSceneTargets)
			{
#if UNITY_6000_0_OR_NEWER
				savedBakeOnSceneLoad = Lightmapping.bakeOnSceneLoad;
				Lightmapping.bakeOnSceneLoad = Lightmapping.BakeOnSceneLoadMode.Never;
#else
				savedGIWorkflowMode = Lightmapping.giWorkflowMode;
          ...
          // Flush accumulated entries from List to array before reading them
				  foreach (var conjunction in assetConjunctions.conjunctions)
					  conjunction.referencedAtInfo.FlushEntries();
            
					SetReferencingEntries(assetConjunctions.conjunctions);
          
          // Periodically release memory accumulated from loaded assets during scanning
				  if (i % 50 == 0 && i > 0)
			...
			finally
			{
				if (hasSceneTargets)
				{
					Lightmapping.Cancel();
#if UNITY_6000_0_OR_NEWER
					Lightmapping.bakeOnSceneLoad = savedBakeOnSceneLoad;
#else
					Lightmapping.giWorkflowMode = savedGIWorkflowMode;
#endif
				}
			}
