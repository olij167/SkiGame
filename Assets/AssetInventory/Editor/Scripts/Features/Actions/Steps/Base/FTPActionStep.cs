...
namespace AssetInventory
{
    ...
    public abstract class FTPActionStep : ActionStep
    {
        ...
        /// Uses lazy loading to defer FTP connections lookup until the parameter is displayed.
        /// </summary>
        protected void AddServerParameter()
        {
            // FTP/SFTP Connection parameter with lazy loading
        ...
        /// <summary>
        /// Builds the list of available FTP/SFTP server connections.
        /// Called on-demand when the parameter options are needed.
        /// </summary>
        ...
        {
            if (param.Name == "Server") return BuildServerOptions();
        ...
        protected FtpWebRequest CreateFtpRequest(FTPConnection connection, string remotePath, string password, bool keepAlive = false)
        {
            ...
            string uri = $"{ftpScheme}://{connection.host}:{connection.port}{IOUtils.NormalizeRemotePath(remotePath)}";
