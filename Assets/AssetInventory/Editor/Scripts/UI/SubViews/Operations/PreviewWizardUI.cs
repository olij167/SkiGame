...
namespace AssetInventory
{
    public sealed class PreviewWizardUI : BasicEditorUI
    {
        ...
        private void Schedule(AssetFile.PreviewOptions state)
        {
            ...
            string query = $"update AssetFile set PreviewState = ? where PreviewState = ? and AssetId in (select Id from Asset where Exclude = false {assetFilter})";
        ...
        private void Schedule(string queryExt = "")
        {
            ...
            // Use subquery syntax compatible with both SQLite and MySQL
            string query = $"update AssetFile set PreviewState = ? where PreviewState in (1,2,3) and AssetId in (select Id from Asset where Exclude = false {assetFilter}) {queryExt}";
            DBAdapter.DB.Execute(query, AssetFile.PreviewOptions.Redo);

            query = $"update AssetFile set PreviewState = ? where PreviewState not in (1,2,3) and AssetId in (select Id from Asset where Exclude = false {assetFilter}) {queryExt}";
        ...
        private void ScheduleByType(string type)
        {
            ...
            string query = $"update AssetFile set PreviewState = ? where PreviewState in (1,2,3) and Type = ? and AssetId in (select Id from Asset where Exclude = false {assetFilter})";
            DBAdapter.DB.Execute(query, AssetFile.PreviewOptions.Redo, type);

            query = $"update AssetFile set PreviewState = ? where PreviewState not in (1,2,3) and Type = ? and AssetId in (select Id from Asset where Exclude = false {assetFilter})";
        ...
        public override void OnGUI()
        {
            ...
            if (_assets != null && _assets.Count > 0 && GUILayout.Button(CommonUIStyles.Content("x", "Clear Selection"), GUILayout.Width(20)))
            {
                _assets = null;
                GeneratePreviewOverview();
            }
            ...
            if (_showTypeBreakdown)
            {
                if (_typeBreakdown != null && _typeBreakdown.Count > 0)
                {
                    ...
                    {
                        EditorGUILayout.BeginHorizontal();
                        EditorGUILayout.LabelField($"{CommonUIStyles.INDENT}{item.Type}", GUILayout.Width(200));
                ...
                {
                    EditorGUILayout.LabelField($"{CommonUIStyles.INDENT}No files scheduled for recreation.", EditorStyles.wordWrappedLabel);
                }

                EditorGUILayout.Space();
                EditorGUILayout.BeginHorizontal();
                EditorGUI.BeginDisabledGroup(AI.Actions.ActionsInProgress);
                if (GUILayout.Button(CommonUIStyles.Content("Clean Queue", "Will remove accidentally scheduled items for which no preview can be created (e.g. cs files)."), GUILayout.ExpandWidth(false)))
                {
                    CleanQueue();
                }
                if (GUILayout.Button(CommonUIStyles.Content("Schedule by Type...", "Schedule all files of a specific type/extension for preview recreation."), GUILayout.ExpandWidth(false)))
                {
                    NameUI nameUI = new NameUI();
                    nameUI.Init("", ScheduleByType, false, "Extension (e.g. prefab)");
            ...
            if (_showAdv)
            {
                ...
                EditorGUI.BeginDisabledGroup(AI.Actions.ActionsInProgress);
                if (GUILayout.Button(CommonUIStyles.Content("Revert to Provided", "Will replace existing recreated previews with those provided originally within the packages."), GUILayout.Width(200)))
                {
                    RestorePreviews();
                }
                if (GUILayout.Button(CommonUIStyles.Content("Clear Queue", "Will remove the scheduled items from the queue again."), GUILayout.Width(200)))
                {
                    ClearQueue();
                }
                EditorGUI.EndDisabledGroup();
            ...
            {
                EditorGUILayout.BeginHorizontal();
                CommonUIStyles.DrawProgressBar((float)_validator.Progress / _validator.MaxProgress, $"Progress: {_validator.Progress}/{_validator.MaxProgress}");
            ...
            {
                EditorGUILayout.BeginHorizontal();
                string text = _assets == null || _assets.Count > 1 ? $"Progress: {_previewPipeline.MainProgress}/{_previewPipeline.MainCount} packages - {_previewPipeline.CurrentSub}" : $"Progress: {_previewPipeline.SubProgress}/{_previewPipeline.SubCount} - {_previewPipeline.CurrentSub}";
                CommonUIStyles.DrawProgressBar((float)_previewPipeline.SubProgress / _previewPipeline.SubCount, text);
                EditorGUI.BeginDisabledGroup(_previewPipeline.CancellationRequested);
                if (GUILayout.Button("Stop", GUILayout.ExpandWidth(false), GUILayout.Height(14)))
                {
            ...
            else
            {
                ...
                if (GUILayout.Button($"Recreate {_scheduledFiles:N0} Scheduled", CommonUIStyles.mainButton, GUILayout.Height(CommonUIStyles.BIG_BUTTON_HEIGHT)))
                {
                    _ = RecreatePreviews();
                }
                EditorGUI.EndDisabledGroup();
                if (GUILayout.Button(CommonUIStyles.Content("Verify", "Inspect all preview images and check for issues like containing Unity default placeholders or shader errors."), GUILayout.Width(buttonWidth), GUILayout.Height(CommonUIStyles.BIG_BUTTON_HEIGHT))) InspectPreviews();
                if (GUILayout.Button("Refresh", GUILayout.Width(buttonWidth), GUILayout.Height(CommonUIStyles.BIG_BUTTON_HEIGHT))) GeneratePreviewOverview();
                EditorGUILayout.EndHorizontal();
            }
        }
        ...
        {
            EditorGUILayout.BeginHorizontal();
            GUILabelWithText($"{CommonUIStyles.INDENT}{label}", $"{count:N0}", labelWidth, tooltip);
