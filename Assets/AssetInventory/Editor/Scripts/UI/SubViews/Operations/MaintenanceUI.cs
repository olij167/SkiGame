...
namespace AssetInventory
{
    public sealed class MaintenanceUI : EditorWindow
    {
        ...
        public void OnGUI()
        {
            ...
            if (GUILayout.Button("Run All", CommonUIStyles.mainButton, GUILayout.ExpandWidth(false), GUILayout.Height(CommonUIStyles.BIG_BUTTON_HEIGHT)))
            {
                ScanAll(false);
            }
            if (GUILayout.Button("Run Only Fast Scans*", GUILayout.ExpandWidth(false), GUILayout.Height(CommonUIStyles.BIG_BUTTON_HEIGHT)))
            {
                ScanAll(true);
            }
            EditorGUI.BeginDisabledGroup(_fixeableItems == 0);
            if (GUILayout.Button("Fix All", GUILayout.ExpandWidth(false), GUILayout.Height(CommonUIStyles.BIG_BUTTON_HEIGHT)))
            {
                FixAll();
            }
            ...
            foreach (Validator validator in _validators.Where(v => v.IsVisible()))
            {
                ...
                switch (validator.CurrentState)
                {
                    ...
                    case Validator.State.Completed:
                        GUI.color = validator.IssueCount == 0 ? Color.green : CommonUIStyles.errorColor;
                            ...
                            if (GUILayout.Button("Show...", GUILayout.ExpandWidth(false)))
                            {
                                switch (validator.Type)
                                {
                                    case Validator.ValidatorType.DB:
                                        LineListWindow.Show("Issue List", validator.DBIssues.Select(i => $"{(string.IsNullOrWhiteSpace(i.Path) ? i.GetDisplayName() : i.Path)} ({i.Id})").OrderBy(s => s));
                                        break;
                    ...
                    case Validator.State.Fixing:
                        GUI.color = CommonUIStyles.errorColor;
