...
namespace AssetInventory
{
    public sealed class WorkspaceUI : BasicEditorUI
    {
        ...
        {
            _searchesListControl = new ReorderableList(_searches, typeof (WorkspaceSearch), true, true, true, true);
        ...
        private void DrawSearchListItem(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            GUI.Label(new Rect(rect.x, rect.y + 2, rect.width, rect.height),
                CommonUIStyles.Content(savedSearch != null ? savedSearch.Name : $"-Unknown Search- ({search.SavedSearchId})"),
                UIStyles.entryStyle);
        }
