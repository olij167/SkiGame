...
namespace AssetInventory
{
    public sealed class DBLocationUI : BasicEditorUI
    {
        ...
        public override void OnGUI()
        {
            ...
            EditorGUILayout.LabelField(EditorUtility.FormatBytes(_dbSize), CommonUIStyles.miniLabelRight, GUILayout.Width(sizeLabelWidth));
            ...
            GUILayout.FlexibleSpace();
            EditorGUILayout.LabelField("free: " + EditorUtility.FormatBytes(_targetSpace), CommonUIStyles.miniLabelRight, GUILayout.Width(sizeLabelWidth));
            GUILayout.EndHorizontal();
            ...
            if (string.IsNullOrEmpty(AI.Config.previewFolder) || string.IsNullOrEmpty(AI.Config.backupFolder) || string.IsNullOrEmpty(AI.Config.cacheFolder))
            {
                ...
                if (string.IsNullOrEmpty(AI.Config.previewFolder))
                {
                    ...
                    EditorGUILayout.LabelField(Paths.GetPreviewFolder().Replace("\\", "/"), EditorStyles.wordWrappedLabel);
                    GUILayout.FlexibleSpace();
                    EditorGUILayout.LabelField(_previewSize < 0 ? "calculating..." : EditorUtility.FormatBytes(_previewSize), CommonUIStyles.miniLabelRight, GUILayout.Width(sizeLabelWidth));
                    GUILayout.EndHorizontal();
                ...
                if (string.IsNullOrEmpty(AI.Config.backupFolder))
                {
                    ...
                    EditorGUILayout.LabelField(Paths.GetBackupFolder().Replace("\\", "/"), EditorStyles.wordWrappedLabel);
                    GUILayout.FlexibleSpace();
                    EditorGUILayout.LabelField(_backupSize < 0 ? "calculating..." : EditorUtility.FormatBytes(_backupSize), CommonUIStyles.miniLabelRight, GUILayout.Width(sizeLabelWidth));
                    GUILayout.EndHorizontal();
                ...
                if (string.IsNullOrEmpty(AI.Config.cacheFolder))
                {
                    ...
                    EditorGUILayout.LabelField(Paths.GetMaterializeFolder().Replace("\\", "/"), EditorStyles.wordWrappedLabel);
                    GUILayout.FlexibleSpace();
                    if (!_ignoreCache) EditorGUILayout.LabelField(_cacheSize < 0 ? "calculating..." : EditorUtility.FormatBytes(_cacheSize), CommonUIStyles.miniLabelRight, GUILayout.Width(sizeLabelWidth));
                    GUILayout.EndHorizontal();
                    ...
                    _ignoreCache = EditorGUILayout.ToggleLeft(CommonUIStyles.Content("Ignore and Delete", "The cache will be recreated on demand whenever needed so it is not crucial to move it as well to save time. You will experience delays upon first access to packages though which were already cached."), _ignoreCache);
                    GUILayout.EndHorizontal();
                    GUILayout.EndVertical();
            ...
            if (GUILayout.Button(_calculating ? "Calculating disk space..." : "Move Database", GUILayout.Height(CommonUIStyles.BIG_BUTTON_HEIGHT)))
            {
