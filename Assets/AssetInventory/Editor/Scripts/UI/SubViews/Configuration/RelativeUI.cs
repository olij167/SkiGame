    ...
    public sealed class RelativeUI : EditorWindow
    {
        ...
        public void OnGUI()
        {
            ...
            if (_disableMode)
            {
                ...
                EditorGUILayout.LabelField(CommonUIStyles.Content("Key"), EditorStyles.boldLabel, GUILayout.Width(width));
                ...
                {
                    GUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(CommonUIStyles.Content("Location to Restore"), EditorStyles.boldLabel, GUILayout.Width(width));
                    _locationIdx = EditorGUILayout.Popup(_locationIdx, _locationsArr);
                    GUILayout.EndHorizontal();

                    GUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(CommonUIStyles.Content("Affected Systems"), EditorStyles.boldLabel, GUILayout.Width(width));
            ...
            else
            {
                ...
                GUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(CommonUIStyles.Content("Location"), EditorStyles.boldLabel, GUILayout.Width(width));
                EditorGUILayout.LabelField(_spec.location);
                GUILayout.EndHorizontal();

                GUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(CommonUIStyles.Content("Key", "Unique key for this location"), EditorStyles.boldLabel, GUILayout.Width(width));
            ...
            {
                CommonUIStyles.DrawProgressBar((float)_currentConversion / _conversionCount, $"{_conversionText}: {_currentConversion + 1}/{_conversionCount}");
            }
            else
            {
                GUILayout.BeginHorizontal();
                if (_disableMode)
                {
                    ...
                    if (GUILayout.Button("Revert relative persistence", CommonUIStyles.mainButton, GUILayout.Height(CommonUIStyles.BIG_BUTTON_HEIGHT))) RevertRelative();
                    EditorGUI.EndDisabledGroup();
                }
                else
                {
                    EditorGUI.BeginDisabledGroup(string.IsNullOrWhiteSpace(_key));
                    if (GUILayout.Button("Start conversion", CommonUIStyles.mainButton, GUILayout.Height(CommonUIStyles.BIG_BUTTON_HEIGHT))) MakeRelative();
                    EditorGUI.EndDisabledGroup();
                }
                GUILayout.EndHorizontal();
            }
