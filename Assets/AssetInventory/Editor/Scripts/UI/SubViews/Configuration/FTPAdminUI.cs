...
namespace AssetInventory
{
    public sealed class FTPAdminUI : BasicEditorUI
    {
        ...
        private ReorderableList _connectionsListControl;
        private Vector2 _detailsScrollPos;

        private ReorderableList ConnectionsListControl
        {
            get
            {
                if (_connectionsListControl == null) InitConnectionsControl();
                return _connectionsListControl;
            }
        }
        ...
        private void InitConnectionsControl()
        {
            _connectionsListControl = new ReorderableList(_connections, typeof (FTPConnection), false, false, true, true);
            _connectionsListControl.drawHeaderCallback = rect => EditorGUI.LabelField(rect, "Connections");
            _connectionsListControl.drawElementCallback = DrawConnectionListItem;
            _connectionsListControl.onAddCallback = OnAddConnection;
            _connectionsListControl.onRemoveCallback = OnRemoveConnection;
            _connectionsListControl.onSelectCallback = OnSelectConnection;
        }
        ...
        private void DrawConnectionList()
        {
            ...
            ConnectionsListControl.DoLayoutList();
        }

        private void DrawConnectionListItem(Rect rect, int index, bool isActive, bool isFocused)
        {
            if (index >= _connections.Count) return;
            FTPConnection conn = _connections[index];
        ...
        {
            AddNewConnection();
            list.index = _connections.Count - 1;
        }

        private void OnRemoveConnection(ReorderableList list)
        {
        ...
        private void OnSelectConnection(ReorderableList list)
        {
            SelectConnection(list.index);
        }
        ...
        private void DrawConnectionDetails()
        {
            ...
            _detailsScrollPos = GUILayout.BeginScrollView(_detailsScrollPos);
            ...
            if (GUILayout.Button("Save Changes", CommonUIStyles.mainButton, GUILayout.Height(30)))
            {
        ...
        private void AddNewConnection()
        {
            ...
            // Reset list control to reflect changes
            _connectionsListControl = null;
        }
