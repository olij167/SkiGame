    ...
    public sealed class AssetConnectionUI : PopupWindowContent
    {
        ...
        public override void OnGUI(Rect rect)
        {
            ...
            if (string.IsNullOrWhiteSpace(_url))
            {
                ...
                EditorGUILayout.HelpBox($"Enter the URL to the asset on the Asset Store. Do not use short URLs but those of the form below or alternatively the Id, e.g. {AI.ASSET_STORE_ID}", MessageType.Info);
                if (GUILayout.Button($"https://assetstore.unity.com/packages/tools/utilities/asset-inventory-{AI.ASSET_STORE_ID}", EditorStyles.linkLabel))
                {
            ...
            if (_resolvedAsset != null)
            {
                ...
                EditorGUILayout.Space();
                if (GUILayout.Button("Connect", CommonUIStyles.mainButton))
                {
