...
namespace PungentFunk.Utilities.Editor.PreviewExport
{
    ...
    public class FontPreviewWindow : EditorWindow
    {
        ...
        private readonly PungentScanSession fontScanSession = new("font-preview-discovery", "Font Preview Discovery");
        private string scanResultSourceBanner = string.Empty;
        ...
        private Vector2 scanIssueScrollPosition;
        ...
        private bool fontRefreshQueued;
        ...
        {
            LoadPrefs();
            status = "Ready. Press Refresh Fonts to discover project fonts.";
            if (PungentScanCache.TryHydrateSession(fontScanSession, out scanResultSourceBanner) && fontScanSession.Result != null)
                status = fontScanSession.Result.StatusMessage;
        }
        ...
        private void OnGUI()
        {
            ...
            DrawToolbar();
            PungentScanGUI.DrawResultHeader(fontScanSession.Result, scanResultSourceBanner);
            PungentScanGUI.DrawIssueList(fontScanSession.Result, ref scanIssueScrollPosition, 90f, "No font discovery issues to display.");
            DrawControls();
        ...
        private void QueueRefreshFontList()
        {
            if (fontRefreshQueued)
                return;

            fontRefreshQueued = true;
            status = "Font refresh queued";
        ...
        {
            PungentScanResult scan = fontScanSession.Begin(PungentScanScope.ProjectAssets, "Project Fonts");
            allFonts.Clear();
            ...
            try
            {
            ...
            #else
                scan.AddIssue(PungentScanSeverity.Info, "TMP previews unavailable", "TMP_PRESENT is not defined, so TextMeshPro font assets were skipped.", null, null, "FONT_TMP_UNAVAILABLE");
            #endif
                ...
                if (allFonts.Count == 0)
                    scan.AddIssue(PungentScanSeverity.Info, "No fonts found", "No Unity Font assets were discovered with the active project query.", null, null, "FONT_NONE_FOUND");
                fontScanSession.Complete(allFonts.Count, filteredFonts.Count, 0, 0, status);
            }
            catch (Exception exception)
            {
                status = "Font discovery failed: " + exception.Message;
                fontScanSession.Fail(exception, status);
            }
        }
