...
namespace PungentFunk.Utilities.Editor.UI
{
    ...
    public sealed class InputPromptIconLibraryAutoFill : EditorWindow
    {
        ...
        private readonly PungentScanSession _scanSession = new PungentScanSession("input-prompt-icon-autofill", "Input Prompt Icon Autofill");
        private string _resultSourceBanner = string.Empty;
        private bool _findDefaultQueued;
        private bool _populateQueued;
        ...
        public static void PopulateSelectedLibrary()
        {
        ...
        public static bool CanPopulateSelectedLibrary()
        {
            return IsInputPromptIconLibrary(Selection.activeObject);
        }

        public static void PopulateDefaultLibrary()
        {
        ...
        private void OnGUI()
        {
            ...
            PungentScanGUI.DrawIssueList(_scanSession.Result, ref _scanIssueScroll, 100f, "No autofill issues to display.");
            DrawPreviewPanel();
        ...
        private void QueueFindDefaultLibrary()
        {
            if (_findDefaultQueued)
                return;

            _findDefaultQueued = true;
            _status = "Default library lookup queued.";
            EditorApplication.delayCall += () =>
            {
                if (this == null)
                    return;

                _findDefaultQueued = false;
                RunFindDefaultLibrary();
                Repaint();
            };
        }
        ...
        private void QueuePopulate()
        {
            ...
            _status = "Populate operation queued.";
            EditorApplication.delayCall += () =>
            {
                if (this == null)
                    return;
        ...
        private void RunFindDefaultLibrary()
        {
            PungentScanResult scan = _scanSession.Begin(PungentScanScope.ProjectAssets, "Find Default Library");
            try
            {
                _library = FindDefaultLibrary();
                _status = _library != null ? $"Found default library: {_library.name}" : "Default library not found.";
                if (_library == null)
                    scan.AddIssue(PungentScanSeverity.Warning, "Default library missing", "No InputPromptIconLibrary_Default or InputPromptIconLibrary asset was found.", null, null, "INPUT_ICON_DEFAULT_MISSING");
                _scanSession.Complete(_library != null ? 1 : 0, _library != null ? 1 : 0, 0, 0, _status);
            }
            catch (Exception exception)
            {
                _status = "Default library lookup failed: " + exception.Message;
        ...
        {
            PungentScanResult scan = _scanSession.Begin(PungentScanScope.ProjectAssets, "Populate Library");
            try
            {
                ...
                if (_lastReport == null)
                {
                    scan.AddIssue(PungentScanSeverity.Error, "Populate failed", "The selected asset could not be populated as an InputPromptIconLibrary.", _library, _library != null ? AssetDatabase.GetAssetPath(_library) : null, "INPUT_ICON_POPULATE_FAILED");
                    _scanSession.Complete(0, 0, 0, 0, _status);
                    return;
                }

                for (int i = 0; i < _lastReport.MissingFiles.Count; i++)
                    scan.AddIssue(PungentScanSeverity.Warning, "Missing sprite match", _lastReport.MissingFiles[i], _library, _library != null ? AssetDatabase.GetAssetPath(_library) : null, "INPUT_ICON_SPRITE_MISSING");

                int scanned = _lastReport.Added + _lastReport.Updated + _lastReport.SkippedExisting + _lastReport.Missing;
                _scanSession.Complete(scanned, _lastReport.Added + _lastReport.Updated, _lastReport.SkippedExisting + _lastReport.Missing, _lastReport.Added + _lastReport.Updated, _status);
            }
            catch (Exception exception)
            {
                _status = "Populate failed: " + exception.Message;
                _scanSession.Fail(exception, _status);
            }
        }
