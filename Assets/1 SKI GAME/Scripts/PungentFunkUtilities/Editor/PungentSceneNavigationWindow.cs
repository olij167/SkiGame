...
namespace PungentFunk.Utilities.Editor.SceneTools
{
    ...
    public sealed class PungentSceneNavigationWindow : EditorWindow
    {
        ...
        public static void FastTravelToSelection()
        {
        ...
        {
            if (PungentUtilityRegistry.CanOpen("fast-travel-sceneview-to-selection", true))
        ...
        private void OnDisable()
        {
            ...
            SetEditorUpdateSubscribed(false);
        ...
        private void DrawFastTravelPanel()
        {
            using (new EditorGUILayout.VerticalScope(UtilityWindowTheme.PanelStyle(UtilityWindowTheme.Blue)))
            {
                ...
                bool drawOverlay = EditorGUILayout.Toggle("SceneView Overlay", _drawOverlay);
                if (drawOverlay != _drawOverlay)
                {
                ...
                for (int i = 0; i < shown.Count; i++)
                {
                    ...
                    using (new EditorGUILayout.VerticalScope(UtilityWindowTheme.PanelStyle(waypoint.color, 0.10f, 0.05f, 4, 2)))
                    {
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            ...
                            if (GUILayout.Button("Up", GUILayout.Width(34f)) && originalIndex > 0)
                                SwapWaypoints(originalIndex, originalIndex - 1);
                            if (GUILayout.Button("Down", GUILayout.Width(48f)) && originalIndex < _waypoints.Count - 1)
                                SwapWaypoints(originalIndex, originalIndex + 1);
        ...
        private void DrawTrackingPanel()
        {
            using (new EditorGUILayout.VerticalScope(UtilityWindowTheme.PanelStyle(UtilityWindowTheme.Purple)))
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    bool followEnabled = GUILayout.Toggle(_followEnabled, "Follow Best Match", EditorStyles.toolbarButton, GUILayout.Width(132f));
                    if (followEnabled != _followEnabled)
                    {
        ...
        {
            SetEditorUpdateSubscribed(_followEnabled);
            SetSceneGuiSubscribed(_drawOverlay);
        }

        private void SetEditorUpdateSubscribed(bool subscribed)
        {
            if (_updateSubscribed == subscribed)
                return;
