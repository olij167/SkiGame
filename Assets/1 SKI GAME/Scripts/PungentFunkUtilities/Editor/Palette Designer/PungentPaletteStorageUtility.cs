...
namespace PungentFunk.Utilities.Editor.Colour
{
    ...
    {
        public struct PaletteAssetInfo
        {
            public string guid;
            public string path;
            public PungentColourPaletteSO palette;
        ...
        public static List<PaletteAssetInfo> FindPaletteAssets()
        {
            string[] guids = AssetDatabase.FindAssets("t:PungentColourPaletteSO");
            var results = new List<PaletteAssetInfo>(guids.Length);
            for (int i = 0; i < guids.Length; i++)
            {
                ...
                PungentColourPaletteSO palette = AssetDatabase.LoadAssetAtPath<PungentColourPaletteSO>(path);
                if (palette == null)
                    continue;

                string displayName = !string.IsNullOrWhiteSpace(palette.paletteName)
                    ? palette.paletteName
                    : !string.IsNullOrWhiteSpace(palette.name) ? palette.name : Path.GetFileNameWithoutExtension(path);

                results.Add(new PaletteAssetInfo
                {
                    guid = guid,
                    path = path,
                    palette = palette,
        ...
        public static PungentColourPaletteSO LoadPaletteByGuid(string guid)
        {
            if (string.IsNullOrWhiteSpace(guid))
                return null;

            string path = AssetDatabase.GUIDToAssetPath(guid);
            return string.IsNullOrEmpty(path) ? null : AssetDatabase.LoadAssetAtPath<PungentColourPaletteSO>(path);
        }

        public static string GetPaletteGuid(PungentColourPaletteSO palette)
        {
            if (palette == null)
                return string.Empty;
