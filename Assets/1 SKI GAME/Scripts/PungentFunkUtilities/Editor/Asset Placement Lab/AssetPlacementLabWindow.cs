...
namespace PungentFunk.Utilities.Editor.Placement
{
    ...
    public sealed class AssetPlacementLabWindow : EditorWindow
    {
        ...
        private const int ScenePreviewDrawCap = 500;
        ...
        private ModularPathSpawner _pathSource;
        ...
        private readonly PungentPlacementSurfaceBrushController _surfaceBrush = new PungentPlacementSurfaceBrushController();
        private string _groupRepairSummary = "No repair run yet.";
        ...
        public static void OpenWithPath(ModularPathSpawner path)
        {
            ...
            window.titleContent = new GUIContent("Asset Placement Lab");
            window.minSize = new Vector2(780f, 540f);
            window._pathSource = path;
            window._areaMode = PungentPlacementAreaMode.ModularPath;
            window._module = LabModule.AreaScatter;
            window._status = path != null ? "Path source received from Spatial Authoring." : "Asset Placement Lab opened.";
            window.Show();
        }
        ...
        private void OnGUI()
        {
            ...
            switch (_module)
            {
                case LabModule.AreaScatter:
                    ...
                    break;
                case LabModule.SurfaceBrush:
                    DrawSurfaceBrushModule();
                    break;
        ...
        private void DrawTopToolbar()
        {
            using (new EditorGUILayout.HorizontalScope(EditorStyles.toolbar))
            {
                ...
                bool drawPreview = _drawPreview;
                UtilityWindowTheme.ToolbarToggle(ref drawPreview, "Scene Preview", UtilityWindowTheme.Cyan);
                if (drawPreview != _drawPreview)
                {
        ...
        private void DrawCorePlacementSection()
        {
            using (new EditorGUILayout.VerticalScope(UtilityWindowTheme.PanelStyle(UtilityWindowTheme.Cyan)))
            {
                ...
                if (_areaMode == PungentPlacementAreaMode.ModularPath)
                {
                    _pathSource = (ModularPathSpawner)EditorGUILayout.ObjectField("Path Source", _pathSource, typeof(ModularPathSpawner), true);
                    if (_pathSource == null)
                        EditorGUILayout.HelpBox("Assign or select a ModularPathSpawner to scatter along a path.", MessageType.Info);
                }
        ...
        private void DrawSurfaceBrushModule()
        {
            if (_surfaceBrush.DrawModuleUI(ref _assetSet, ref _ruleSet, ref _outputParent))
            {
                RefreshSceneGuiSubscription();
                RequestSceneRepaint();
            }
            _status = _surfaceBrush.Summary;
        }
        ...
        private void DrawResultsSection()
        {
            using (new EditorGUILayout.VerticalScope(UtilityWindowTheme.PanelStyle(UtilityWindowTheme.Neutral)))
            {
                ...
                if (_previewResult.candidates.Count > ScenePreviewDrawCap)
                    EditorGUILayout.HelpBox($"Scene preview drawing is capped to {ScenePreviewDrawCap} candidates to keep the editor responsive.", MessageType.Info);
        ...
        private void DrawGroupManagerModule()
        {
            ...
            using (new EditorGUILayout.VerticalScope(UtilityWindowTheme.PanelStyle(UtilityWindowTheme.Teal)))
            {
                ...
                if (_showGroups)
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        if (UtilityWindowTheme.TintedButton("Revalidate Selected Group", UtilityWindowTheme.Teal, GUILayout.Height(24f)))
                            _groupRepairSummary = PungentPlacementGroupRepairUtility.RevalidateSelectedGroup();
                        if (UtilityWindowTheme.TintedButton("Remove Orphan Markers", UtilityWindowTheme.Amber, GUILayout.Height(24f)))
                        {
                            _groupRepairSummary = PungentPlacementGroupRepairUtility.RemoveOrphanMarkers();
                            ScanSceneMarkers();
                        }
                        if (UtilityWindowTheme.TintedButton("Select Generated Children", UtilityWindowTheme.Blue, GUILayout.Height(24f)))
                            _groupRepairSummary = PungentPlacementGroupRepairUtility.SelectGeneratedChildren();
                        if (UtilityWindowTheme.TintedButton("Copy Report", UtilityWindowTheme.Neutral, GUILayout.Height(24f)))
                            EditorGUIUtility.systemCopyBuffer = _groupRepairSummary;
                    }

                    using (new EditorGUILayout.HorizontalScope())
                    {
                        if (UtilityWindowTheme.TintedButton("Find Missing Prefabs", UtilityWindowTheme.Purple, GUILayout.Height(24f)))
                            _groupRepairSummary = PungentPlacementGroupRepairUtility.FindMissingSourcePrefabs();
                        if (UtilityWindowTheme.TintedButton("Replace Source Prefab", UtilityWindowTheme.Purple, GUILayout.Height(24f)))
                            _groupRepairSummary = PungentPlacementGroupRepairUtility.ReplaceSourcePrefabForSelection();
                        if (UtilityWindowTheme.TintedButton("Re-align To Surface", UtilityWindowTheme.Green, GUILayout.Height(24f)))
                            _groupRepairSummary = PungentPlacementGroupRepairUtility.RealignSelectedMarkersToSurface(_ruleSet);
                    }

                    EditorGUILayout.HelpBox(_groupRepairSummary, MessageType.None);
        ...
        private void GeneratePreview()
        {
            if (_areaMode == PungentPlacementAreaMode.ModularPath)
            {
                if (PungentPlacementPathIntegrationUtility.TryGeneratePathPreview(_pathSource, _assetSet, _ruleSet, _seed, _count, out _previewResult, out _status))
                {
                    RefreshSceneGuiSubscription();
                    RequestSceneRepaint();
                }
        ...
        private Bounds ResolveAreaBounds()
        {
            ...
            if (_areaMode == PungentPlacementAreaMode.ModularPath)
            {
                if (PungentPlacementPathIntegrationUtility.TryBuildPathBounds(_pathSource, out Bounds bounds))
                    return bounds;
            }
        ...
        private void OnSceneGUI(SceneView sceneView)
        {
            ...
            if (_module == LabModule.SurfaceBrush)
            {
                if (_surfaceBrush.HandleSceneGUI(sceneView, _assetSet, _ruleSet, _outputParent, _seed))
                    _status = _surfaceBrush.Summary;
                return;
            }
            ...
            int drawn = 0;
            for (int i = 0; i < _previewResult.candidates.Count && drawn < ScenePreviewDrawCap; i++)
            {
                PungentPlacementCandidate c = _previewResult.candidates[i];
                if (c == null)
                    continue;
        ...
        {
            SetSceneGuiSubscribed(_drawPreview && (_module == LabModule.AreaScatter || (_module == LabModule.SurfaceBrush && _surfaceBrush.IsActive)));
        }
