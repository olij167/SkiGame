...
namespace SkiGame.Progression
{
    ...
    public static class PhoneHUDMountainSummaryBuilder
    {
                ...
                for (int i = 0; i < poiRegistry.Current.Count; i++)
                {
                    ...
                    switch (poi.type)
                    {
                        ...
                        case POIType.SkiLift:
                            ...
                            {
                                bool canUse = skiPassMgr == null || skiPassMgr.CanUseLift(lift.RequiredPassLevel);
        ...
        public static string BuildLiftMapSummary(PlayerStatsProfile profile, LiftLine lift, string displayName, SkiPassManager skiPassMgr)
        {
            ...
            bool accessible = skiPassMgr == null || skiPassMgr.CanUseLift(lift.RequiredPassLevel);
        ...
        private static string GetLiftPassRequirementLabel(LiftLine lift, SkiPassManager skiPassMgr)
        {
            ...
            var cfg = skiPassMgr != null ? skiPassMgr.Config : null;
            if (cfg != null)
            {
                var level = cfg.Get(lift.RequiredPassLevel);
                if (level != null)
                    return $"{level.displayName} (L{lift.RequiredPassLevel})";
            }

            return $"Pass Level {lift.RequiredPassLevel}";
        }
