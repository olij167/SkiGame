...
namespace SkiGame.Progression
{
    ...
    public sealed class SkiLessonDirector : MonoBehaviour
    {
        ...
        private bool _moveStepSawForwardLean;
    ...
    var lifts = FindObjectsOfType<LiftLine>();
        ...
        private void ResetPerStepState()
        {
            ...
            _moveStepSawForwardLean = false;

        }

        private void HandleStepEntered()
        {
            if (_currentStep == LessonStep.Lean)
            {
                _sawForwardLean = _moveStepSawForwardLean;
            }
        ...
        private float GetCurrentStepProgress01()
        {
            switch (_currentStep)
            {
                ...
                case LessonStep.Wedge:
                    return (skiController.IsRiderGrounded &&
                            skiController.LeftLegInput >= 0.35f &&
                            skiController.RightLegInput >= 0.35f) ? 1f : 0f;
        ...
        private string GetStepTitle(LessonStep step)
        {
            switch (step)
            {
                ...
                case LessonStep.Wedge: return "Wedge";
        ...
        private string GetStepBody(LessonStep step)
        {
            switch (step)
            {
                ...
                case LessonStep.Wedge:
                    return $"Press {GetBindingDisplay("LeftSki")} and {GetBindingDisplay("RightSki")} together to wedge your skis.";
        ...
        private string GetStepHint(LessonStep step)
        {
            switch (step)
            {
                ...
                case LessonStep.Wedge:
                    return $"Hold {GetBindingDisplay("LeftSki")} and {GetBindingDisplay("RightSki")} at the same time.";
