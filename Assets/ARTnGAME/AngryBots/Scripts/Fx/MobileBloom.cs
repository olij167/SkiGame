...
public class MobileBloom : MonoBehaviour {
		...
		bool Supported () {
			return ( bloomShader.isSupported);
		}
		...
		void OnRenderImage (RenderTexture source,RenderTexture destination) {		
			...
			//RenderTexture tempRtLowA = RenderTexture.GetTemporary (source.width / 4, source.height / 4, rtFormat);
			//RenderTexture tempRtLowB = RenderTexture.GetTemporary (source.width / 4, source.height / 4, rtFormat);
			RenderTexture tempRtLowA = RenderTexture.GetTemporary (source.width / 4, source.height / 4);
			RenderTexture tempRtLowB = RenderTexture.GetTemporary (source.width / 4, source.height / 4);
			...
			//Graphics.Blit (source, destination, apply, QualitySettings.GetQualityLevel() > 3 ? 4 : 0); //v2.3
			Graphics.Blit (source, destination, apply, QualityManager.quality > QualityManager.Quality.Medium ? 4 : 0);//v2.3
