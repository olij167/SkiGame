...
public class LaserScope : MonoBehaviour {
		...
		void Start() {
			lRenderer = gameObject.GetComponent <LineRenderer>();	//gameObject.GetComponent (LineRenderer) as LineRenderer;	//v2.3
			//aniTime = 0.0f;
		...
		void Update () {
			...
			GetComponent<Renderer> ().material.mainTextureOffset = 
				new Vector2 (GetComponent<Renderer>().material.mainTextureOffset.x + Time.deltaTime * aniDir * scrollSpeed, GetComponent<Renderer>().material.mainTextureOffset.y);
			...
			lRenderer.startWidth = lRenderer.endWidth = aniFactor; //v2.3
			...
			if (hitInfo.transform) {
				...
				GetComponent<Renderer> ().material.mainTextureScale = 
					new Vector2 (0.1f * (hitInfo.distance), GetComponent<Renderer>().material.mainTextureScale.y);
				...
				if (pointer) {
					...
					pointer.transform.eulerAngles = new Vector3 (90.0f,pointer.transform.eulerAngles.y,pointer.transform.eulerAngles.z); //v2.3
				}
			}
			else {
				if (pointer)
				...
				GetComponent<Renderer> ().material.mainTextureScale = 
					new Vector2 (0.1f * (maxDist), GetComponent<Renderer>().material.mainTextureScale.y);
