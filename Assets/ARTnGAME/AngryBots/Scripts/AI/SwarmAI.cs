...
public class SwarmAI : MonoBehaviour {
			...
			for (int i = 0; i<buzz.Count; i++) {
				if (buzz[i].motor.enabled) {
					...
					if (spawnDir.sqrMagnitude < 0.01f) {
						...
						buzz[i].transform.GetComponent<Rigidbody>().linearVelocity = Vector3.zero;
		...
		void UpdateAttack () {
			...
			for (int i = 0; i<count; i++) {
				...
				ParticleSystem.MainModule mainMod = buzz[i].electricBall.main;//v2.3
				ParticleSystem.MinMaxCurve curve = mainMod.startSize;
				curve.constantMin = 0.30f * visibleCharged; //v2.3
				curve.constantMax = 0.45f * visibleCharged;
		...
		IEnumerator DoElectricArc (BuzzerData buzz) {
			...
			while (Time.time < stopTime) {
				...
				buzz.electricArc.sharedMaterial.mainTextureOffset = new Vector2 (Random.value, buzz.electricArc.sharedMaterial.mainTextureOffset.y);//v2.3
				//yield; //v2.3
				yield return true;
			}
