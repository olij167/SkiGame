...
namespace Artngame.SKYMASTER.EtherealVolumetrics
{
    ...
    public static class VolumetricUtils
    {
        ...
        //v0.1
        public static void GetPixelCoordToViewDirWSRG(UniversalCameraData cameraData, Vector4 resolution, ref Matrix4x4[] transforms)
        {
            // if (cameraData.xr.singlePassEnabled)
            // {
            //     for (int viewIndex = 0; viewIndex < viewCount; ++viewIndex)
            ...
            // else
            // {
            transforms[0] = ComputePixelCoordToWorldSpaceViewDirectionMatrixRG(cameraData, resolution);
            // }
        }
        public static void GetPixelCoordToViewDirWSRG(UniversalCameraData cameraData, Vector4 resolution, ref Matrix4x4 transform)
        {
            transform = ComputePixelCoordToWorldSpaceViewDirectionMatrixRG(cameraData, resolution);
        }
        internal static Matrix4x4 ComputePixelCoordToWorldSpaceViewDirectionMatrixRG(UniversalCameraData cameraData, Vector4 resolution)
        {
            var camera = cameraData.camera;
            SetCameraMatricesRG(cameraData, out var viewMatrix, out var cameraProj, out var viewProjMatrix, out var invViewProjMatrix);
            ...
            //v28d
            bool isUsingTAA = cameraData.antialiasing == AntialiasingMode.TemporalAntiAliasing;

            if (useGenericMatrix && !isUsingTAA)
            {
                var viewSpaceRasterTransform = new Matrix4x4(
                    new Vector4(2.0f * resolution.z, 0.0f, 0.0f, -1.0f),
                    new Vector4(0.0f, -2.0f * resolution.w, 0.0f, 1.0f),
        ...
        internal static void SetCameraMatricesRG(UniversalCameraData cameraData, out Matrix4x4 viewMatrix, out Matrix4x4 projMatrix, out Matrix4x4 viewProjMatrix, out Matrix4x4 invViewProjMatrix)
        {
            var camera = cameraData.camera;
            viewMatrix = camera.worldToCameraMatrix;
