...
namespace Artngame.SKYMASTER {
    ...
    public class connectSuntoVolumeFogURP : MonoBehaviour
    {
        ...
        [Tooltip("Unity 2022 new Blitter, requires PATCH inside the Resources folder with the Fog shader")]
        public bool Unity2022 = false; //requires PATCH inside the Resources folder with the Fog shader
        ...
        //v2.0
        [Tooltip("Use only fog, need latest shader v1.5 and up")]
        public bool useOnlyFog = false;
        [Tooltip("Use infinite local lights, need latest shader v1.5 and up")]
        ...
        public float blendVolumeLighting = 0.03f;//0; //v5.4.2
        [Tooltip("Volumetric lighting Sampling Steps")]
        public float LightRaySamples = 12;//8;

        //v1.3.0
        [Tooltip("Volumetric Fog Power, zero applies only volume lights")]
        public float GlobalFogPower = 1;
        [Tooltip("Volumetric Fog Noise")]
        public float GlobalFogNoisePower = 1;
        [Tooltip("Volumetric Lighting Noise")]//Set _Thickness to zero if VolumeLightNoisePower != 0
        public float VolumeLightNoisePower = 0;
        [Tooltip("Use 3D Noise Texture instead of calculate noise in shader")]
        public bool useTexture3DNoise = false;
        [Tooltip("3D Noise Texture")]
        public Texture3D _NoiseTex1;
        [Tooltip("Local Light Volume Attenuation")]//v1.4.3
