Shader "Unlit/VolumeFogSRP_FORWARD_URP Legacy 1 NEW IMPOSTOR"
{
	Properties
	{
		...
		_SunPosition("sun pos", Color) = (111, 11,339, 11)
			...
			attenExponent("attenExponent", Float) = 1.0

			_BaseTex("Base (RGB)", 2D) = "white" {}
			//_FogTex("Base (RGB)", 2D) = "white" {}

			useDX12("use DX12 (1)", Float) = 0.0
	}
...
#define sampler_MainLightShadowmapTexture sampler_LinearClampCompare //Unity 2022
float attenExponent;
...
#define SAMPLES_INT 16
	...
	float GlobalFogPower;
	float GlobalFogNoisePower;
	float VolumeLightNoisePower;
	int useTexture3DNoise;
	...
	float3 VolumeFog(float3 sourceImg, float dist, float3 ray, float3 rayStart, float3 WorldPosition, float2 texcoord, float depth)
	{
		...
		for (int i = 0; i < lightsArrayLength; i++)
		{
			...
			if (lightDir.x != 0 || lightDir.y != 0 || lightDir.z != 0) {
				...
				if ( diff1<diff2) { //if behind obstacle, zero intensity
					///lightPower2 = 0;
					...
					//lightPower2 = abs(diff1+0.1)*(1.35/pow(diff2 - diff1 + 1,2))*lightPower2;
					//lightFalloff = 6 + lightFalloff *o*0.5;
				}else{
					//lightPower2 = abs(diff1+0.1)*1811/pow(diff2 - diff1 + 1,2);
					//lightPower2 = abs(diff1+0.1)*1111/pow(diff2 - diff1 + 1,2);
					//lightPower2 += abs(diff1+0.1)*2/pow(diff2 - diff1 + 1,2);
				}
				//lightFalloff = 2;
				//lightPower2 = 100/(diff1 - diff2);
				//lightPower2 = lightPower2 + 522*pow(diff1 - diff2,2.4);
				
				//lightPower2 = lightPower2 - 111;
		...
		if (lightControlA.x != 0) {
			...
			for (int i = 0; i < steps; ++i)
			{
				...
				//COOKIE
				float3 cookieDirColor = SampleMainLightCookie(pos);
				//directionalLight.color *= cookieDirColor;
				...
				//COOKIE
				attenuate = attenuate * cookieDirColor;
...
//v2.0
#if defined(INFINITE) /////////////// HANDLE ALL LOCAL LIGHTS
					...
					//LIGHT 1
						...
						float3 cookieColor = SampleAdditionalLightCookie(k, pos);
						...
						if ((lightCount < 0 && distToRayStartA < abs(lightCount*0.001)) || (lightCount > k && _visibleLightsCount > k + 1)) {//v1.9.9.5 if (_visibleLightsCount > 1) {//v1.9.9.8
							if (controlByColor == 0 || (controlByColor == 1 &&
								...
								sourceImg = sourceImg + lightPower3 * lightControlA.z * o * 0.04*sourceImg * light.color * pow(light.distanceAttenuation,attenExponent) * light.shadowAttenuation / powDIVIDE;//*15
							}
						}
					}
#else // NOT INFINITE
					...
					//COOKIE
					//#if defined(_LIGHT_COOKIES)
					float3 cookieColor = SampleAdditionalLightCookie(0, pos);
					light.color *= cookieColor;
					//#endif
#else
...
//LIGHT 2
					if (lightCount > 1 && _visibleLightsCount > 2) { //v1.9.9.5
						...
						//COOKIE
						//#if defined(_LIGHT_COOKIES)
						float3 cookieColor = SampleAdditionalLightCookie(1, pos);
						light.color *= cookieColor;
						//#endif
#else
						...
						if (controlByColor == 0 || (controlByColor == 1 &&
							...
							sourceImg = sourceImg + lightPower3 * lightControlA.w * o * 0.04*sourceImg * light.color * pow(light.distanceAttenuation,attenExponent) * light.shadowAttenuation / powDIVIDE;
						}
					}
					//LIGHT 3
					if (lightCount > 2 && _visibleLightsCount > 3) {
						...
						//COOKIE
						//#if defined(_LIGHT_COOKIES)
						float3 cookieColor = SampleAdditionalLightCookie(2, pos);
						light.color *= cookieColor;
						//#endif
#else
						...
						if (controlByColor == 0 || (controlByColor == 1 &&
							...
							sourceImg = sourceImg + lightPower3 * lightControlB.x * o * 0.04*sourceImg * light.color * pow(light.distanceAttenuation,attenExponent) * light.shadowAttenuation / powDIVIDE;
						}
					}
					//LIGHT 4
					if (lightCount > 3 && _visibleLightsCount > 4) {
						...
						//COOKIE
						//#if defined(_LIGHT_COOKIES)
						float3 cookieColor = SampleAdditionalLightCookie(3, pos);
						light.color *= cookieColor;
						//#endif
#else
						...
						if (controlByColor == 0 || (controlByColor == 1 &&
							...
							sourceImg = sourceImg + lightPower3 * lightControlB.y * o * 0.04*sourceImg * light.color * pow(light.distanceAttenuation,attenExponent) * light.shadowAttenuation / powDIVIDE;
						}
					}
					...
					if (lightCount > 4 && _visibleLightsCount > 5) {
						...
						//COOKIE
						//#if defined(_LIGHT_COOKIES)
						float3 cookieColor = SampleAdditionalLightCookie(4, pos);
						light.color *= cookieColor;
						//#endif
#else
						...
						if (controlByColor == 0 || (controlByColor == 1 &&
							...
							sourceImg = sourceImg + lightPower3 * lightControlB.z * o * 0.04*sourceImg * light.color * pow(light.distanceAttenuation,attenExponent) * light.shadowAttenuation / powDIVIDE;
						}
					}
					if (lightCount > 5 && _visibleLightsCount > 6) {
						...
						//COOKIE
						//#if defined(_LIGHT_COOKIES)
						float3 cookieColor = SampleAdditionalLightCookie(5, pos);
						light.color *= cookieColor;
						//#endif
#else
						...
						if (controlByColor == 0 || (controlByColor == 1 &&
							...
							sourceImg = sourceImg + lightPower3 * lightControlB.w * o * 0.04*sourceImg * light.color * pow(light.distanceAttenuation,attenExponent) * light.shadowAttenuation / powDIVIDE;
						}
					}
					#endif
	...
	//	float depth =  Linear01Depth(zsample * (zsample < 1.0)); //////// CHANGE 2 URP
		...
		float3 pointToCamera = (wpos);// * 0.47; //v2.1 float3 pointToCamera = (wpos - _WorldSpaceCameraPos);// * 0.47;
		int steps = 2;
		float stepCount = 1;
		...
		if (GlobalFogNoisePower != 0 || VolumeLightNoisePower != 0) {//v1.3.0
			//int noise3D = 0;
			half4 noise;
			half4 noise1;
			half4 noise2;
			if (noise3D == 0) {
				...
				if (useTexture3DNoise == 1) {
					float3 coord = wpos + float3(_Time.y * 1 * _NoiseSpeed.x,
						_Time.y * _NoiseSpeed.y,
						_Time.y * _NoiseSpeed.z);
					float2 uv = input.texcoord * 24.0 + float2(0.2, 1) * _Time.y * 0.0001;
					float w = 0.102* _Time.y;
					float3 uvw = coord;//float3(uv,w);
					const float baseFreq = 1e-5;
					float4 uvw1 = float4(uvw * _NoiseScale * baseFreq, 0);
					uvw1.xyz += _NoiseSpeed.xyz * _Time.x*0.0001;
					float n1 = tex3Dlod(_NoiseTex1, uvw1 * 610).a;
					uvw1.xyz += _NoiseSpeed.xyz * _Time.x*0.0005;
					float n2 = tex3Dlod(_NoiseTex1, uvw1 * 310).a;
					//float n = n1 * _NoiseAmp1*AlphaFactor + n2 * _NoiseThickness;//v3.5.3
					float n = (n1 + n2 * 0.7) * _NoiseThickness;//v3.5.3
					//n = saturate(n + _NoiseBias);
					n = saturate(n);
					noise = float4(n, n1, n, 1);
					noise1 = float4(n1, n, n, 1);
					noise2 = float4(n, n1, n1, 1);
				}
				else {
			...
			//v1.3.0
			noiseer = pow(abs((noise.r + 1 * noise1.r + _NoiseDensity * noise2.r * 1)), 1.2)*0.3 * GlobalFogNoisePower;
		}//v1.3.0
		...
		if (blackBackground == 1 && useDX12 == 0) {
		...
		//v1.3.0
		float g = 0;
		if (GlobalFogPower != 0) {//v1.3.0

			g = ComputeDistance(input.ray, depth) - _DistanceOffset;//v1.3.0
			if (doDistanceA == 1) {
			...
			//g = g * pow(abs((noise.r + 1 * noise1.r + _NoiseDensity * noise2.r * 1)), 1.2)*0.3; //v1.3.0
			g = g * GlobalFogPower * noiseer; //v1.3.0
		}
...
//#else
		...
		//v1.3.0
		if (VolumeLightNoisePower == 0) {
			VolumeLightNoisePower = 1;
			noiseer = 1;
		}

		///v1.3.0
		float4 shadows = 0;//v1.3.0

		#if !defined(ONLY_FOG)//v2.0
		if (blendVolumeLighting != 0) {//v1.3.0

			float normalizeMag = length(posToCameraA);
			shadows = half4(VolumeFog(result.rgb, normalizeMag, (posToCameraA * noiseer * VolumeLightNoisePower) / normalizeMag, _WorldSpaceCameraPos, wpos, input.texcoord, depth), result.a);
...
#pragma fragment FragGrey
			...
			Pass // 7
		{
			...
			prevGI = lerp(prevGI, clamp(prevGI, minPrev, maxPrev), 0.25* _TemporalGain);
		...
		////////////// TEMPORAL - https://github.com/cdrinmatane/SSRT - MIT LICENSE
			...
			Pass // 20
			{
				Name "TemporalReproj2"
				HLSLPROGRAM
				//#pragma vertex vert
				#include "BlitEthereal.hlsl"//v0.2
				...
				float4 frag(VaryingsA  input) : SV_Target
				{
					...
					//float2 motionVectors = SAMPLE_TEXTURE2D_X_LOD(_CameraMotionVectorsTexture,sampler_CameraMotionVectorsTexture, float4(uv.xy,0,0),0).xy;
					float2 reprojCoord = input.uv.xy;// -motionVectors.xy;

					//NEW TAA
					// //https://github.com/CMDRSpirit/URPTemporalAA/tree/86f4d28bc5ee8115bff87ee61afe398a6b03f61a
					//temporal reprojection
					float d0 = depth;//sampleDepth(i.uv);
					float d01 = (d0 * (_ProjectionParams.z - _ProjectionParams.y) + _ProjectionParams.y) / _ProjectionParams.z;
					float3 pos = float3(reprojCoord * 2.0 - 1.0, 1.0);
				...
				//prevGI = clamp(prevGI, minPrev, maxPrev); ///BASIC
					//prevGI = clamp(prevGI, minPrev, maxPrev)*0.75 + prevGI*0.25; ///BASIC
			...
			////// END BLEND	
				...
				half4 color = SAMPLE_TEXTURE2D_X_LOD(_BlitTexture, sampler_LinearClamp, uv, _BlitMipLevel); //sampler_LinearRepeat, uv, _BlitMipLevel);
			...
			ENDHLSL
					...
					///////////// END UNITY 2022.2 URP 14


					/////// SSMS
							// 10: Prefilter  /// 25 (offset = plus 15)
				Pass
			{
				ZTest Always Cull Off ZWrite Off
				CGPROGRAM
				...
				#include "SSMS.cginc"
				#pragma vertex vertN
				#pragma fragment frag_prefilterA
				#pragma target 3.0
				ENDCG
			}
				// 11: Prefilter with anti-flicker /// 26
				Pass
			{
				ZTest Always Cull Off ZWrite Off
				CGPROGRAM
				...
				Pass
			{
				...
				#include "SSMS.cginc"
				#pragma vertex vertN
				#pragma fragment frag_downsample1A
				#pragma target 3.0
				ENDCG
			}
				// 13: First level downsampler with anti-flicker /// 28
				Pass
			{
				ZTest Always Cull Off ZWrite Off
				CGPROGRAM
				...
				// 14: Second level downsampler /// 29
				Pass
			{
				ZTest Always Cull Off ZWrite Off
				CGPROGRAM
				#include "SSMS.cginc"
				#pragma vertex vertN
				#pragma fragment frag_downsample2A
				#pragma target 3.0
				ENDCG
			}
				// 15: Upsampler /// 30
				Pass
			{
				...
				CGPROGRAM
				#include "SSMS.cginc"
				#pragma vertex vert_multitexN
				#pragma fragment frag_upsampleA
				#pragma target 3.0
				ENDCG
			}
				// 16: High quality upsampler/// 31 
				Pass
			{
				ZTest Always Cull Off ZWrite Off
				CGPROGRAM
				...
				Pass
			{
				...
				#include "SSMS.cginc"
				#pragma vertex vert_multitexN
				#pragma fragment frag_upsample_finalN
				#pragma target 3.0
				ENDCG
			}
				// 18: High quality combiner /// 33
				Pass
			{
				ZTest Always Cull Off ZWrite Off
				CGPROGRAM
				...
				Pass
			{
				...
				#include "SSMS.cginc"
				#pragma vertex vert_multitexN
				#pragma fragment frag_upsample_final2G
				#pragma target 3.0
				ENDCG
			}
				/////// END SSMS

				Pass //35 BLIT BACKGROUND
			{
				Name "ColorBlitPasss"
				HLSLPROGRAM
				#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl"
				...
				// Inverts the sampled color
				//return half4(1, 1, 1, 1) - color;
				//return tex2D(_BaseTex, input.texcoord);

				half4 fog = tex2D(_FogTex, input.texcoord);
				half4 main = tex2D(_BaseTex, input.texcoord);
				return float4(fog.rgb + main.rgb, 1) * 1;
			...
			// 11: Prefilter with anti-flicker /// 26 //36
				Pass
			{
				ZTest Always Cull Off ZWrite Off
				HLSLPROGRAM
				...
				#include "BlitEthereal.hlsl"//v0.2
				#pragma vertex Vert
				#pragma fragment frag_prefilterAA

				sampler2D _MainTexA;
				float2 _MainTexA_TexelSize;
				...
				sampler2D _FogResult;
				float2 _FogResult_TexelSize;
				...
				half4 frag_prefilterAA(VaryingsB  i) : SV_Target//   v2f_img i) : SV_Target
				{

					//return  tex2D(_FogResult, i.texcoord)*2 ;//float4(1,0,0,1);
					//return tex2D(_MainTexA, i.texcoord)*100;

					float2 uv = i.texcoord + _FogResult_TexelSize.xy * _PrefilterOffs;

				#if ANTI_FLICKER
					float3 d = _FogResult_TexelSize.xyx * float3(1, 1, 0);
					half4 s0 = SafeHDR(tex2D(_FogResult, uv));
					half3 s1 = SafeHDR(tex2D(_FogResult, uv - d.xz).rgb);
					half3 s2 = SafeHDR(tex2D(_FogResult, uv + d.xz).rgb);
					half3 s3 = SafeHDR(tex2D(_FogResult, uv - d.zy).rgb);
					half3 s4 = SafeHDR(tex2D(_FogResult, uv + d.zy).rgb);
					...
					// SMSS 
					//half depth = tex2D(_FogTex, i.texcoord); // Deferred
					// half depth = tex2D(_FogTex, float2(i.texcoord.x, 1 - i.texcoord.y)); // Forward
					half depth = tex2D(_FogTex, float2(i.texcoord.x,  i.texcoord.y)); // Forward
				...
				Pass //37 BLIT FOG plus BACKGROUND
				{
					Name "ColorBlitPasssA"
					HLSLPROGRAM
					#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl"
					//#include "Packages/com.unity.render-pipelines.core/Runtime/Utilities/Blit.hlsl"
					#include "BlitEthereal.hlsl"//v0.2
					...
					float2 _FogResult3_TexelSize;
					sampler2D _FogResult2; sampler2D _FogResult;
					sampler2D _FogResult3;
					sampler2D _FogTex;
					sampler2D _FadeTex;
					...
					half3 UpsampleFilterA(float2 uv)
					{
						...
						float4 d = 0.016 * float4(1, 1, -1, 0) * _SampleScale;

						half3 s;
						s = DecodeHDR(tex2D(_FogResult3, uv - d.xy));
						s += DecodeHDR(tex2D(_FogResult3, uv - d.wy)) * 2;
						s += DecodeHDR(tex2D(_FogResult3, uv - d.zy));

						s += DecodeHDR(tex2D(_FogResult3, uv + d.zw)) * 2;
						s += DecodeHDR(tex2D(_FogResult3, uv)) * 4;
						s += DecodeHDR(tex2D(_FogResult3, uv + d.xw)) * 2;
						...
						float4 d = 0.016 * float4(-1, -1, +1, +1) * (_SampleScale * 0.5);

						half3 s;
						s = DecodeHDR(tex2D(_FogResult3, uv + d.xy));
						s += DecodeHDR(tex2D(_FogResult3, uv + d.zy));
						s += DecodeHDR(tex2D(_FogResult3, uv + d.xw));
						s += DecodeHDR(tex2D(_FogResult3, uv + d.zw));
					...
					//half4 fog = tex2D(_FogResult3, input.texcoord);
					//half4 main = tex2D(_FogResult2, input.texcoord);

					half4 base = tex2D(_FogResult, input.texcoord);
					half3 blur = UpsampleFilterA(input.texcoord) * 1;

					//return tex2D(_FogResult3, input.texcoord);//
					//return float4(blur.rgb,1);//
						...
						// SMSS
						//half depth = tex2D(_FogTex, input.texcoord); // Deferred
						 half depth = tex2D(_FogTex, float2(input.texcoord)); // Forward
						depth = AdjustDepth(depth);
						...
						//blur.a=0.5;	
					//	return lerp(base, half4(6*	blur*base.rgb+blur*3, 1) * (1 / _Radius) + 0*base/4, clamp(depth, 0, _Intensity));
						return lerp(base, half4(blur, 1) * (1 / _Radius), clamp(depth, 0, _Intensity));


						//return float4( main.rgb  , 1) * 1;
