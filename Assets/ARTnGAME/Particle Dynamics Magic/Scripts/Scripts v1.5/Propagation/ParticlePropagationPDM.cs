...
[ExecuteInEditMode()]
	public class ParticlePropagationPDM : MonoBehaviour {
	
		//v2.0 - pooling
		public bool Use_pooling = false; // enable the new pooling behavior for decals, also handle particle restart
		//public bool Reset_particles = false;
		[HideInInspector]
		public List<Transform> Item_pool = new List<Transform>();
	...
	void Start () {
		...
		//v1.8
		public bool Free_objects = false;
		public bool Lerp_motion = false;
		public bool Follow_part_vel = false;
		...
		public float X_offset_factor=0.005f;//v1.8
		...
		//v1.6
		//Add look at normal
		public bool normal_On_Y=true;
		private ControlCombineChildrenPDM Combiner;
	...
	public List<Vector3> Registered_paint_positions; 
		...
		public bool enable_freeze=false; //use when fire goes out, inject FreezeBurnControlPDM script, increase counter if exists
	...
	void LateUpdate () {
		...
		if(p11 == null){return;}
			...
			if(Parent_OBJ!=null){
				if(enable_combine){
					if(Combiner==null){
						Combiner = Parent_OBJ.GetComponent(typeof(ControlCombineChildrenPDM)) as ControlCombineChildrenPDM;
						//make sure auto disable is on
						if(Combiner!=null){
							Combiner.Auto_Disable=true;
						}else{
							Parent_OBJ.AddComponent(typeof(ControlCombineChildrenPDM));
							Combiner = Parent_OBJ.GetComponent(typeof(ControlCombineChildrenPDM)) as ControlCombineChildrenPDM;
							Combiner.Auto_Disable=true;
						}
					}
			...
			for (int i=Registered_paint_positions.Count-1;i>=0 ;i--){
				...
				else if( 1==1 & (Application.isPlaying & (!keep_alive | (enable_overides & (LocalOverrides[i] == 1 | LocalOverrides[i] == 3) ) ) & Emitter_objects[i].gameObject.tag != "Flamer" & ( (Time.fixedTime - Registered_paint_times[i]) > stay_time) ) ) //v1.4
				{
						...
						if(gameobject_mode){
								...
								//v2.0
								if(Use_pooling){
									//keep the item disabled in the pool list
									Gameobj_instances[i].gameObject.SetActive(false);
									//if(Reset_particles){ // handle any attached particles - reset to restart their effect when enabled

									//}
									Item_pool.Add(Gameobj_instances[i]);
								}else{
									Destroy(Gameobj_instances[i].gameObject);
								}
			...
			for(int j=0;j<Emitter_objects.Count;j++){
				if( Emitter_objects[j] != null & Flammable_objects[i]!= null  ){
					...
					if(result != Vector3.zero){
											if(Emitter_objects.Count > (p11.main.maxParticles/2) ){//v2.1
							//do nothing
						}else{
										...
										if(is_close_to_other_point_on_object==0){ 
											...
											Registered_paint_positions.Add(result);
													...
													//v1.6
													if(!gameobject_mode){
														Registered_paint_size.Add (p11.main.startSize.constant+Random_size_factor);
													}
			...
			for(int j=0;j<Flamer_objects.Length;j++){
				...
				for(int j=0;j<Projectors.Count;j++){
							...
							Quaternion Rotation_at_origin =  Projectors[j].transform.rotation;
								...
								int max_positions = (int)p11.main.maxParticles/2;//v2.1
			...
			if(hit.collider.gameObject.tag == "PPaint"){
				...
				if(Emitter_objects!=null){
					...
					if(!Erase_mode){
							...
							Registered_paint_positions.Add(hit.point);
									Updated_Registered_paint_positions.Add(hit.point);
										Particle_color.Add(new Vector4(p11.main.startColor.color.r,p11.main.startColor.color.g,p11.main.startColor.color.b,p11.main.startColor.color.a)); //v2.3
									Registered_paint_rotations.Add(hit.normal);
									Registered_paint_times.Add (Time.fixedTime);
				...
				for( int j=0; j<Stencil.width ;j=j+(int)Stencil_divisions.x){
					for( int k=0; k<Stencil.height ;k=k+(int)Stencil_divisions.y){						
						...
						if(1==0 | tex_col.a > 0.9f){							
							...
							maxemitter_count = ((int)p11.main.maxParticles/2)+1;//v2.1
							if(Emitter_objects!=null){
				...
				if(Preview_mode | Application.isPlaying){
					...
					if(Registered_paint_positions!=null){
						...
						for(int i=0;i<Registered_paint_positions.Count;i++){
								...
								if(Use_pooling){
									if(Item_pool.Count > 0){//if items exist, grab, enable and remove from the list

										TEMP = Item_pool[0].gameObject;
										TEMP.transform.position = Registered_paint_positions[i];
										TEMP.SetActive(true);
										Item_pool.RemoveAt(0);
										//Debug.Log ("Done");
									}else{
										TEMP = Instantiate(Gameobj,Registered_paint_positions[i],Quaternion.identity)as GameObject;
										//Debug.Log ("Zero");
									}
								}else{
									TEMP = Instantiate(Gameobj,Registered_paint_positions[i],Quaternion.identity)as GameObject;
							...
							if(gameobject_mode & (Application.isPlaying | Preview_mode) ){
								if(counter_regsitered < Gameobj_instances.Count & !follow_particles){									
									...
									if(Free_objects){
										...
										if(Lerp_motion){
											Gameobj_instances[counter_regsitered].position = Vector3.Lerp(Gameobj_instances[counter_regsitered].position, FIND_moved_pos, grav_factor*Time.deltaTime);
										}else{
									...
									if(!look_at_direction){
											if(normal_On_Y){
												...
												Gameobj_instances[counter_regsitered].LookAt(Gameobj_instances[counter_regsitered].position + relative*FIND_moved_normal_toZERO, Vector3.right);
											}
									}else{
...
//												}
										...
										if(Gameobj_instances.Count > i | 1==1){ 
											...
											//Gameobj_instances[counter_regsitered].position = ParticleList[i].position;
												...
												if(Free_objects){
													...
													if(Lerp_motion){
														Gameobj_instances[counter_regsitered].position = Vector3.Lerp(Gameobj_instances[counter_regsitered].position, ParticleList[i].position, 0.1f*grav_factor*Time.deltaTime);
													}else{
														Gameobj_instances[counter_regsitered].position = ParticleList[i].position;
													}
												}
											...
											//v.1.8
											if(Follow_part_vel){
												Gameobj_instances[counter_regsitered].LookAt(Gameobj_instances[counter_regsitered].position + ParticleList[i].velocity);
											}else{
												...
												if(Motion_vec.magnitude > 0.1f & Motion_vec.magnitude < 99999f){
													...
													New_rot = Quaternion.LookRotation(1*Motion_vec);
														...
														//if(counter_regsitered !=0){
															//Debug.DrawLine(Gameobj_instances[counter_regsitered].position, (Gameobj_instances[counter_regsitered].position + ParticleList[i].velocity*0.02f));
														//}
												}
							...
							if(keep_alive & !(enable_overides & (LocalOverrides[counter_regsitered] == 1 | LocalOverrides[counter_regsitered] == 3) ) & let_loose){
								...
								if(Color_effects){
									if(Particle_color[counter_regsitered].w < 257){
										if(Keep_color){
											ParticleList[i].startColor=new Color(Particle_color[counter_regsitered].x,Particle_color[counter_regsitered].y,Particle_color[counter_regsitered].z,Particle_color[counter_regsitered].w);
										}else{
											if(ParticleList[i].remainingLifetime > keep_in_position_factor*ParticleList[i].startLifetime){
												if(Lerp_color){
							...
							if(variant_size){
									...
									if(gameobject_mode & Vary_gameobj_size){
										...
										ParticleList[i].startSize = Registered_paint_size[counter_regsitered];
									}
								...
								if( 1==1 & (!keep_alive & (is_ice)) | (enable_overides & LocalOverrides[counter_regsitered]==1 ) ){
										...
										if(1==1){
											...
											if(gameobject_mode & Vary_gameobj_size){
												...
												if( ParticleList[i].startSize > 0){

												if( 1==1 & (enable_overides & LocalOverrides[counter_regsitered]==1 ) ){
														ParticleList[i].startSize = Registered_paint_size[counter_regsitered] - fast_melt_speed ;
												}
												else{
														ParticleList[i].startSize = Registered_paint_size[counter_regsitered] - melt_speed ;
												}												
											}
											...
											if(gameobject_mode & Vary_gameobj_size){
												...
												if(Gameobj_instances[counter_regsitered].localScale.x>0){
													...
													Registered_paint_size[counter_regsitered] = ParticleList[i].startSize;
											}
										}								
