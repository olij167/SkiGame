...
namespace Artngame.PDM {
	...
	public class SplinerP : MonoBehaviour {

		//v2.1
		public bool Multithreaded = false;
		public bool DefineRotations = false;

		public List<LandSplineNode[]> Keep_Curve;
		public LandSplineNode[] Curve1;
		...
		void Start () {
			...
			#region CALC CURVE
			LandSplineNode[]  SplinePoints1 = SplinePoints.ToArray();
			...
			if (Keep_Curve == null ){Keep_Curve = new List<LandSplineNode[]>();}
		...
		void Draw_multithreaded(int i, LandSplineNode[]  SplinePoints1, int Control_points_Count, bool realtime){
			float detail = CurveQuality;
			if (detail < 1) {
				detail = 1;
			}
		...
		//
		public void Draw_Curve(int i, LandSplineNode[]  SplinePoints1, int Control_points_Count, bool realtime)
		{
			//if ((Multithreaded && Thead_finished) | !Multithreaded) {
			//Curve.Clear ();
			//Curve.AddRange (Curve1);
		...
		[SerializeField]
		public List<LandSplineNode> Curve = new List<LandSplineNode>(); 

		private int count_steps;
		private int count_steps2;
		...
		void Update () {
			...
			if(Game_Mode_On){
				...
				if(Add_point){
					...
					SplinePoints.Add(new LandSplineNode(Moved_vector_to_last_point,Quaternion.identity));
					...
					if(With_manip){
						control_points_children[control_points_children.Count-1].AddComponent(typeof(DragTransformPDM));
					}
				...
				if(SplinePoints.Count > 2 ){
					...
					if (Add_mid_point) {
						...
						if(Add_in_Segment > 0 & Add_in_Segment < SplinePoints.Count){
							...
							SplinePoints.Insert(Add_in_Segment, new LandSplineNode(Moved_vector_to_last_point,Quaternion.identity));							
							GameObject SPHERE_SMALL = GameObject.CreatePrimitive(PrimitiveType.Sphere);							
							...
							if(With_manip){
								control_points_children[Add_in_Segment].AddComponent(typeof(DragTransformPDM));
							}							
...
//							}
		...
		[SerializeField()]
		public List<LandSplineNode> SplinePoints = new List<LandSplineNode>();
	...
	#region Node
	[System.Serializable]
	public class LandSplineNode {

		public LandSplineNode ( Vector3 pos, Quaternion rot) {
			this.position = pos;
			this.rotation = rot;
		}
