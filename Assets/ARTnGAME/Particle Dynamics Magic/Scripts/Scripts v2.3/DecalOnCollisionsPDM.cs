...
public class DecalOnCollisionsPDM : MonoBehaviour {
	
		public int maxDecals = 30;
		public float startSize = 1;

		public int maxHits = 1;
		public bool destroyAfterMaxHits = false;
		int hitCount = 0;
...
//			}		
		...
		public List<ParticlePropagationPDM> systems_To_override;//use to cache scripts
		public float cut_off_dist=0.1f; //max distance to check other system for painted positions

		void Update(){
			if (hitCount >= maxHits) {
				if (destroyAfterMaxHits) {
...
//		}
	...
	//void OnParticleCollision1(GameObject other){
			...
			if(((propagator_exists & Propagator_use_collisions & !Procedural_check) | !propagator_exists ) ){
				...
				if(propagator_exists){
					particle_count = maxDecals;
                        ...
                        //for(int j=0; j<collisionEvents.Length;j++){
                        for (int j = 0; j < 1; j++) //v2.3 - remove extra collisions
                        { 
                         //for(int j=0; j<collision.contacts.Length;j++){
                         //for(int k=0; k<collisionEvents[j].Length;k++){

                            //overrides - v2.3
                            ContactPoint collisionEvent = collision.contacts[j];
								...
								for(int i=0;i< systems_To_override.Count;i++){									
									...
									if(systems_To_override[i].enable_overides){
										for(int m=0;m<systems_To_override[i].Registered_paint_positions.Count;m++){
											...
											if(Vector3.Distance(collisionEvent.point,systems_To_override[i].Registered_paint_positions[m]) < cut_off_dist)
											{												
						...
						if(Propagator.Registered_paint_positions!=null ){
							...
							if(Propagator.Registered_paint_positions.Count > ( particle_count)){
								//do nothing

							}else{
								

											if(collisionEvent.otherCollider != null){//if(collisionEvents[j][k].collider !=null){ //v2.1
												if(collisionEvent.otherCollider.gameObject !=null){
										...
										//if(Random.Range(1,Propagator.propagation_chance_factor+1) == Propagator.propagation_chance_factor){
											...
											is_close_to_other_point_on_object =1;//v2.3
										...
										if(Decal_per_mat){
											...
											for(int k1 = 0;k1< Mat_names.Count;k1++){
															if(collisionEvent.otherCollider.gameObject.GetComponent<Renderer>().material.name.Contains(Mat_names[k1])){
										...
										if(is_close_to_other_point_on_object == 1 & enter_on_mat){
														Propagator.Registered_paint_positions.Add(collisionEvent.point);
														Propagator.Registered_paint_rotations.Add(collisionEvent.normal);
								Propagator.Registered_paint_times.Add (Time.fixedTime);
														...
														hitCount++;
															...
															Propagator.Registered_paint_size.Add (startSize+Propagator.Random_size_factor);
															}

														Propagator.Updated_Registered_paint_positions.Add(collisionEvent.point);
												...
												if(Propagator.gameobject_mode & (Application.isPlaying) ){
													if(Propagator.Gameobj_instances.Count < (Propagator.particle_count)){
														//GameObject TEMP = Instantiate(Propagator.Gameobj,Propagator.Registered_paint_positions[Propagator.Registered_paint_positions.Count-1],Quaternion.identity)as GameObject;
																...
																//Debug.Log("hit ground and add 3D decal in " + collisionEvent.point + "instances count="+Propagator.Gameobj_instances.Count);
														...
														if(propagator_exists){
															if(Propagator.normal_On_Y){
																		TEMP.transform.rotation = Quaternion.FromToRotation(Vector3.up,collisionEvent.normal);
															}else{
																		TEMP.transform.LookAt(TEMP.transform.position + collisionEvent.normal, Vector3.right);
															}
														}else{
															if(normal_On_Y){
																		TEMP.transform.rotation = Quaternion.FromToRotation(Vector3.up,collisionEvent.normal);
												...
												Propagator.PaintTypes.Add(ParticlePropagationPDM.PaintType.ParticleCollision);
														...
														Propagator.Emitter_objects.Add(collisionEvent.otherCollider.gameObject.transform);
														Propagator.Registered_initial_positions.Add(collisionEvent.otherCollider.gameObject.transform.position);
														Propagator.Registered_initial_scale.Add (collisionEvent.otherCollider.gameObject.transform.localScale);

														Propagator.Registered_initial_rotation.Add(collisionEvent.otherCollider.gameObject.transform.rotation);

															if(Add_effects.Count > 0){
																if(Add_effects[Decal_ID] !=null){
																Add_effects[Decal_ID].Registered_paint_positions.Add(collisionEvent.point);
																Add_effects[Decal_ID].Registered_paint_rotations.Add(collisionEvent.normal);
																	Add_effects[Decal_ID].Registered_paint_times.Add (Time.fixedTime);
																	
																	//v1.6
																	if(!Add_effects[Decal_ID].gameobject_mode){
																	Add_effects[Decal_ID].Registered_paint_size.Add (startSize+Add_effects[Decal_ID].Random_size_factor);
																	}
																	
																Add_effects[Decal_ID].Updated_Registered_paint_positions.Add(collisionEvent.point);
																...
																Add_effects[Decal_ID].Emitter_objects.Add(collisionEvent.otherCollider.gameObject.transform);
																Add_effects[Decal_ID].Registered_initial_positions.Add(collisionEvent.otherCollider.gameObject.transform.position);
																Add_effects[Decal_ID].Registered_initial_scale.Add (collisionEvent.otherCollider.gameObject.transform.localScale);
																	
																Add_effects[Decal_ID].Registered_initial_rotation.Add(collisionEvent.otherCollider.gameObject.transform.rotation);
																}
															}
								...
								for (int i=0;i< Propagator.Updated_Registered_paint_positions.Count;i++){
										...
										if( Vector3.Distance(collisionEvent.point,Propagator.Updated_Registered_paint_positions[i]) < (0.5f*Propagator.brush_size))
									{
