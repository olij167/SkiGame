...
public class ThirdPersonCameraPDM : MonoBehaviour {
	...
	bool snap = false;
	ThirdPersonControllerPDM controller;
	...
	void Awake ()
	{
		...
		if (_target)
		{
			controller = _target.GetComponent<ThirdPersonControllerPDM>();
		}
	...
	{
		float a1 = Mathf.Repeat(a, 360);
		float b1 = Mathf.Repeat(b, 360);

		return Mathf.Abs(b1 - a1);
	}
	...
	void SetUpRotation (Vector3 centerPos,Vector3 headPos)
	{
		...
		Ray centerRay = cameraTransform.GetComponent<Camera>().ViewportPointToRay(new Vector3(.5f, 0.5f, 1));
