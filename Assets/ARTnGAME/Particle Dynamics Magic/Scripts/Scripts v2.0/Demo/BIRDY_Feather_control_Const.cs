...
{
	public class BIRDY_Feather_control_Const : MonoBehaviour
	{
		...
		void OnGUI()
		{
			...
			//		if (GUI.Button(new Rect(10+(1*100),10,150,20), "Toggle focus")){
			//
			...
			//		}
			//
			//		if (GUI.Button(new Rect(10+(1*100),10+1*20,150,20), "Toggle fire")){
			//			if(fire_mode==0){
			...
			if (GUI.Button(new Rect(10, 10 + (1 * 20) - 10, 100, 20), "Walk"))
			{
				...
				ParticleSystem.MainModule main = Fire_particles.main;//v2.3
				main.maxParticles = 0;
				toggle_growth = false;
			...
			if (GUI.Button(new Rect(disp_x + 10, disp_y + 10 + (3 * 20), 100, 20), "Fall"))
			{
				if (cur_target == 0)
				{
					Animator.GetComponent<Animation>().Play("jump_pose");
				}
				else
				{
					Animator.GetComponent<Animation>().Play("jump_pose");
			...
			if (GUI.Button(new Rect(disp_x + 10, disp_y + 10 + (4 * 20), 100, 20), "Idle"))
			{
				if (cur_target == 0)
				{
			...
			if (grow_particles)
			{
				...
				if (main.maxParticles < 1000 & !toggle_growth)
				{
					//Fire_particles.maxParticles=main.maxParticles+5;
					main.maxParticles = main.maxParticles + 5;

					if (main.maxParticles > 350)
					{
						if (Curve.constant < 3)
						{
							//Fire_particles.startSize=Fire_particles.startSize+0.1f;
							Curve.constant = Curve.constant + 0.1f;
						}
					}

					if (main.maxParticles >= 999)
					{
						toggle_growth = true;
					}
				}
				...
				{
					//grow_particles = false;
					//Fire_particles.maxParticles = main.maxParticles-10;
			...
			Lerpy = GUI.HorizontalSlider(new Rect(10, 10 + (22 * 20), 150, 20), SkinnedController.Lerp_ammount, -19f, 19f);
