...
{
	public class PDM_Demo_TURBULENCE : MonoBehaviour
	{
					...
					if (Settings_mounted)
					{
						...
						if (SplineParticlePlacer != null)
						{
							...
							if (PROJECTILES_COUNTER == 50)
							{
								...
								Particle_size = GUI.HorizontalSlider(new Rect(10, 440 - 100, 160, 17), SplineParticlePlacer.gameObject.GetComponent<ParticleSystem>().main.startSize.constant, 0.5f, 5f);
						...
						{ //gameobject, on fire
							...
							if (Gameobject_SCATTER != null)
							{
								...
								if (PROJECTILES_COUNTER == GAMEOBJECT_SETTING)
								{
									...
									//v2.3
									ParticleSystem.MainModule main = Gameobject_SCATTER.gameObject.GetComponent<ParticleSystem>().main;

									Gravity = GUI.HorizontalSlider(new Rect(2 * BOX_WIDTH + X_DIST, BOX_HEIGHT + 480, 150, 17), main.gravityModifier.constant, 0, 5);
									main.gravityModifier = Gravity;//v2.3
								}
