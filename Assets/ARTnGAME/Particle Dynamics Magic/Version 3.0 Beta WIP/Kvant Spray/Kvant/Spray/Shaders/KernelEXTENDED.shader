...
//
Shader "Hidden/Kvant/Spray/KernelEXTENDED"
{
    Properties
    {
        _PositionBuffer ("-", 2D) = ""{}
    ...
    float4 frag_update_velocity(v2f_img i) : SV_Target
    {
		...
		//v0.1 - add interactor power
		//if (length(p.xyz - _InteractorPos.xyz) < _InteractorPower.z * 3) {
		if (length(p.xyz - _InteractorPos.xyz) < _InteractorPower.z * 0.95) {
			//v += 0.0001*0.000001*(pow(p.xyz - _InteractorPos.xyz, 7) * _InteractorPower.x * _InteractorPower.z);
			v.xyz += 0.2*(1/pow(p.xyz - _InteractorPos.xyz, 3)) * _InteractorPower.x * _InteractorPower.z*_Acceleration.w;
		}
