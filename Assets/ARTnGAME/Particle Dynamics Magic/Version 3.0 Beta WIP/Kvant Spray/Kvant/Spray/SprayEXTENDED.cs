...
namespace Artngame.PDM.Kvant
{
    [ExecuteInEditMode]
    //[AddComponentMenu("Kvant/Spray")]
    public partial class SprayEXTENDED : MonoBehaviour
    {
        ...
        {
            if (_bulkMesh == null)
                _bulkMesh = new BulkMeshEXTENDED(_shapes);
            else
        ...
        void SwapBuffersAndInvokeKernels()
        {
            ...
            {
                if(i == activeInteractor)
                {
                    SprayInteractor interacScript = interactors[i].GetComponent<SprayInteractor>();
                    if (interacScript != null)
                    {
                        _kernelMaterial.SetVector("_InteractorPos", interactors[i].position);// + transform.position);
                        if (!interacScript.isExplosion)
                        {
                            _kernelMaterial.SetVector("_InteractorPower", new Vector4(interacScript.interactPower, interacScript.interactExponent, interactors[i].localScale.x, 1));
                        }
                        else
                        {
                            _kernelMaterial.SetVector("_InteractorPower", new Vector4(interacScript.interactPower, interacScript.interactExponent, interactors[i].localScale.x, 1));
                    ...
                    activeInteractor++;
                    if(activeInteractor > interactors.Count-1)
                    {
                        activeInteractor = 0;
                    }
                }
