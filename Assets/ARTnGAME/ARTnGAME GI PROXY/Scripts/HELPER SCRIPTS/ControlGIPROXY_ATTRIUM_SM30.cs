	...
	void Start () {
		...
		void OnGUI(){
			...
			if(intro_ended){
				...
				if(GUI.Button(new Rect(0,100-5,110,35),"Toggle GUI "+GUI_ON)){
				...
				if (enable_GUI) {
					if (GUI.Button (new Rect (600, 10, 200, 40), "Toggle Floor")) {
			...
			if(GI_PROXY_script!=null){
				...
				if(GUI.Button(new Rect(400,50,200,40),GI_ON_POINT)){
					...
					//v3.4
					//if (volLigthToggle) {
						//Debug.Log (GI_PROXY_SPOT_script.enabled);
						//if ((GI_PROXY_SPOT_script.enabled || GI_PROXY_POINT_script.enabled)) {
						if(!GI_PROXY_script.enabled){
							Camera.main.gameObject.GetComponent<AtmosphericScatteringDeferredSkyMaster> ().enabled = false;
						} else {
							Camera.main.gameObject.GetComponent<AtmosphericScatteringDeferredSkyMaster> ().enabled = true;
						}
						//volLigthToggle = false;
					//}
				...
				///// Rotate sun
					...
					GUI.TextArea(new Rect(0,460-28,200,25),"Bloom");
					//Debug.Log (RenderSettings.ambientLight.r);
					Bloomer.bloomIntensity = GUI.HorizontalSlider(new Rect(0,460,200,30),Bloomer.bloomIntensity ,0f,0.65f);

					//v3.4
					GUI.TextArea(new Rect(0,490-28+25,200,25),"Sun ray Intensity");
					//Debug.Log (RenderSettings.ambientLight.r);
					VolumeLight.heightRayleighIntensity = GUI.HorizontalSlider(new Rect(0,490+25,200,30),VolumeLight.heightRayleighIntensity ,0.0f,0.8f);

					//v3.4.3
//					if (GUI.Button (new Rect (0, 425 + 60 + 60, 200, 20), "Toggle Tentacles")) {
//
//					}
				
				GUI.TextArea(new Rect(0,160-28,200,25),"Sun light");
				//Debug.Log (RenderSettings.ambientLight.r);
				intensity_current = GUI.HorizontalSlider(new Rect(0,160,200,30),GetComponent<Light>().intensity,0.1f,2.65f);
				...
				if(GI_PROXY_POINT_script.enabled){
						if(GUI.Button(new Rect(0,445+60+60,200,20),ONOFF_mount+ " point light")){
							if(PointLight.gameObject.transform.root.gameObject == Procedural_sphere.gameObject){
				...
				if(Tentacles!=null){
					...
					if(GUI.Button(new Rect(0,465+60+60,200,20),"Toggle Quality")){
						if(QualitySettings.GetQualityLevel() == 4){
								QualitySettings.IncreaseLevel(false);

							//v3.4.3
							VolumeLight.occlusionSamples = AtmosphericScatteringSkyMaster.OcclusionSamples.x164;
								...
								//v3.4.3
								VolumeLight.occlusionSamples = AtmosphericScatteringSkyMaster.OcclusionSamples.x64;
							}
						}
						

					if(GUI.Button(new Rect(0,485+60+60,200,20),"Toggle Atrium")){
				...
				if(GI_PROXY_script.Degrade_speed == 0.2f){//0.135
					HIGH="low";
				}

				if(1==1){
				if(GUI.Button(new Rect(200,100,200,40),"Toggle GI power "+HIGH) & 1==1){
					...
					if(GI_PROXY_script.Degrade_speed == 0.165f){
						GI_PROXY_script.Degrade_speed = 0.2f;//= 0.135f;
					}else{
						GI_PROXY_script.Degrade_speed = 0.165f;
					}

					if(GI_PROXY_script.Appear_speed == 0.15f){
						GI_PROXY_script.Appear_speed = 0.2f;//= 0.135f;
					}else{
						GI_PROXY_script.Appear_speed = 0.15f;
					}

					if(GI_PROXY_script.Bounce_Intensity == 0.26f){
						GI_PROXY_script.Bounce_Intensity = 0.32f;// = 0.22f;
					}else{
						GI_PROXY_script.Bounce_Intensity = 0.26f;
					}

					if(GI_PROXY_script.Color_change_speed == 12){
						GI_PROXY_script.Color_change_speed = 18;
					}else{
						GI_PROXY_script.Color_change_speed = 12;
					}

						if(GI_PROXY_script.Bounce_Range == 27){
							GI_PROXY_script.Bounce_Range = 32;
						}else{
							GI_PROXY_script.Bounce_Range = 27;
						}

						if(GI_PROXY_script.min_density_dist == 8){
							GI_PROXY_script.min_density_dist = 6;
						}else{
							GI_PROXY_script.min_density_dist = 8;
						}

						if(GI_PROXY_script.max_hit_light_dist == 1){
							GI_PROXY_script.max_hit_light_dist = 3;
						}else{
							GI_PROXY_script.max_hit_light_dist = 1;
						}
