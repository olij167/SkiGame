...
namespace Artngame.SKYMASTER.SEGISRP
{
	[ExecuteInEditMode]
	...
	public class SEGI : MonoBehaviour
	{
		...
		public int numMipLevels = 6; //const int numMipLevels = 6;
		...
		void Start()
		{
			...
			//MINE - solve issue with flickering in optimized mode and when using skybox
			if (Camera.main != null && Application.isPlaying)
			{
				//prevFlags = Camera.main.clearFlags;
			}
		}
		...
		//void OnPostRender(){
		//if (Camera.main != null && Application.isPlaying) {			
		//Camera.main.clearFlags = prevFlags;
		//}	
		//}
		CameraClearFlags prevFlags;
		...
		void OnPreRender()
		{
			...
			//MINE
			updateGIcounter++;
			if (!updateGI || (updateGIevery > 1 && updateGIcounter < updateGIevery && Time.fixedTime > 1))
			{//MINE			
				return;
			}
			else
			...
			if (renderState == RenderState.Voxelize)
			{
				...
				//float voxelTexel = (1.0f * voxelSpaceSize) / (int)voxelResolution * 0.5f;			//Calculate the size of a voxel texel in world-space units //v4.2a
