...
namespace Artngame.SKYMASTER.SEGISRP
{
	...
	public class SEGICascadedEditor : Editor
	{
		...
		SerializedProperty updateGIevery;//v4.2a
		...
		const string presetPath = "Assets/ARTnGAME/ARTnGAME GI PROXY/SEGI/Resources/Cascaded Presets";//v4.2a
		...
		//bool showReflectionProperties = true; //v4.2a
		...
		void OnEnable()
		{
			...
			updateGIevery = serObj.FindProperty("updateGIevery"); //v4.2a
		...
		public override void OnInspectorGUI()
		{
			...
			if (showMainConfig)
			{
				...
				EditorGUILayout.PropertyField(updateGIevery, new GUIContent("Update Every", "Update rate optimization, Lower the rate the effect updates in run time")); //v4.2a
