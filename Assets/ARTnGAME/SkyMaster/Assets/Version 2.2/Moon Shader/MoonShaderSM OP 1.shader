...
Shader "SkyMaster/MoonShader OP 1" {
    Properties {
        ...
        _Bump ("Bump", 2D) = "white" {}
			 _ColorTint("Light tint Color", Color) = (0,0,0,0)
    }
    SubShader {
    Tags
        {
 ...
 fixed Bumped_lightingN = dot( UnpackedN, i.dir);
		...
		float4 aaa = res * 0.05 + saturate(res) * 1 + _ColorTint * (0.2 + _ColorTint.a - res)*_ColorTint.a; //return res;

        return float4(aaa.rgb, 1);
    }
    ENDCG
