...
public class AtmosphericScatteringSkyMaster : MonoBehaviour {
	...
	void Awake() {
		...
		if (SkyManager != null) {
			if (Sun == null) {
                Sun = SkyManager.SUN_LIGHT;//SkyManager.SunObj; //v4.2
            }
		}
	...
	void OnEnable() {
		...
		UpdateStaticUniforms();
            ...
            {
               // Debug.LogErrorFormat("Unexpected: AtmosphericScattering.instance already set (to: {0}). Still overriding with: {1}.", instance.name, name); //v4.8
            }
		instance = this;
	...
	void UpdateDynamicUniforms() {
		...
		Shader.SetGlobalMatrix("u_SMSkyDomeRotation",
			...
			* Matrix4x4.TRS(Vector3.zero, Quaternion.identity, new Vector3(1f, skyDomeVerticalFlip ? -1f : 1f, 1f))                   
            ...
            //} else {
            Shader.SetGlobalVector ("u_SMSunDirection", hasSun ? -Sun.transform.forward : Vector3.down); //v4.7 put minus to solve issue with moon
			//}
