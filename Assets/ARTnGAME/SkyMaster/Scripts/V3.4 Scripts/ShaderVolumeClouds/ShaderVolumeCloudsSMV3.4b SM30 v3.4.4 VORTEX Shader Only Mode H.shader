...
Shader "SkyMaster/ShaderVolumeClouds-Desktop-SM3.0 v3.4.4 VORTEX ScriptLess H" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
				...
				float worldPos_y = 3360+_CutHeight;
               ...
               // float4 paintTexture1 = tex2D(_PaintMap,UVs1+_CloudMap1_ST);
				...
				float3 betaM  = totalMie(lambdaA, KA, fog_depth) * mieCoefficient;
             	...
             	//SCATTER
            	float3 Difference = i.worldPos.xyz - _WorldSpaceCameraPos;Difference.y=worldPos_y - _WorldSpaceCameraPos.y;//v0.1
					...
					float cosTheta = dot(normalize(Difference), _WorldSpaceLightPos0);// (float3(sunPosition.x, sunPosition.y, sunPosition.z)) ); //v3.3					
					...
					float diffuseFactor = max(dot(normalN.xyz, _WorldSpaceLightPos0),0);  // float3(sunPosition.x, sunPosition.y, sunPosition.z) ), 0);  //v3.3
...
//             	float PosTDiff = i.worldPos.y*PosDiff;
             	float DER1 = -(worldPos_y-0)*1;
             	float PosTDiff = worldPos_y*1;
             	if(worldPos_y > change_h){             		
...
//             	}
      ...
      //       	float shaper = (_Transparency-0.48+0.0)*((DER1*saturate((_Coverage+cloudTexture1.a)))-Texture2.a);		////////////////////////////////////////
             	...
             	float shaper = ((_Transparency+_TranspOffset)-0.48+0.0)*((DER1*(((_Coverage+_CoverageOffset)+cloudTexture1.a)))*Texture2.a) * _Coverage; //v5.2.4
    ...
    //         	float Lerpfactor = (ColDiff+(_ShadowColor.a*(dot(lightDirect,normalN)-1)*ColDiff));
               ...
               // if(_Mode==0){
	                change_h =10;   
	                PosDiff =  Thickness*0.0006*(worldPos_y-change_h);  
	                PosTDiff = worldPos_y*PosDiff;          
	             	DER1 = -(worldPos_y+0)*PosDiff;
...
//                float4 Fcolor = float4(saturate(endColor*0.9 + 0.1*endColor*_FogFactor*1*diff2*_FogColor*0.000000001*1),saturate(shaper - 0.01*(_HorizonFactor*0.00001*diff2*1)  )) ;
				...
				Fcolor.rgb = Fcolor.rgb * (pow(_LightColor0,0.2 + sunPosition.w*0.1))+ float3(1, 1, 1) * localLightPower;
