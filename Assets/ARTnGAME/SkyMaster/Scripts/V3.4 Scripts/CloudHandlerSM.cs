...
public class CloudHandlerSM : MonoBehaviour {
		...
		public bool rotateMultiQuadC = false;
		...
		[HideInInspector]
		public bool EnableInsideClouds = false;

		public bool FogFromSkyGrad = false;
...
//		public AnimationCurve IntensityDiff = new AnimationCurve (new Keyframe (0, -0.5f), new Keyframe (0.75f, -0.5f), new Keyframe (0.85f, -1.25f), new Keyframe (0.9f, -0.25f), new Keyframe (1, -0.5f));
//		public AnimationCurve IntensityFog = new AnimationCurve (new Keyframe (0, 5f), new Keyframe (0.75f, 10f), new Keyframe (0.85f, 40f), new Keyframe (0.9f, 20f), new Keyframe (1, 5f));
//		public AnimationCurve IntensitySun = new AnimationCurve (new Keyframe (0, 0.078f), new Keyframe (0.75f, 0.118f), new Keyframe (0.85f, 0.04f), new Keyframe (0.9f, 0.118f), new Keyframe (1, 0.078f));
		...
		public enum CloudPreset {Custom, ClearDay};//, Storm, Mobile};
		...
		public bool autoRainbow = false;
		public float rainbowApperSpeed = 1;
		bool was_rain = false;
		float was_rain_end_time;
		public float rainbowTime = 60;
		//public float rainbowIntensity = 0;
		public float rainboxMaxIntensity = 0.7f;
		public float rainboxMinIntensity = 0.0f;
	...
	//	Color NightCloudColor= new Color(50f/255f,50f/255f,50f/255f,255f/255f);
	//	Color StormCloudColor= new Color(35f/255f,35f/255f,35f/255f,255f/255f);	

		//public Vector3 Cloud_center;
	...
	//	Transform cam_transf;
		...
		public bool HasScatterShader = false;
		public bool UpdateScatterShader = false;
...
//		float ExposureBias = 1.0;
		...
		if (Application.isPlaying) {
			...
			//Cloud_center = this.transform.position;
			...
			if (SkyManager != null) {	
					if (cloudFlatMat != null) {	
						UpdateCloudParams (true, cloudFlatMat);
					}
					if (cloudSidesMat != null) {	
						UpdateCloudParams (true, cloudSidesMat);	
					}
					if (cloudSidesAMat != null) {	
						UpdateCloudParams (true, cloudSidesAMat);	
					}
					if (cloudSidesBMat != null) {	
						UpdateCloudParams (true, cloudSidesBMat);	
					}
					if (cloudRotMat != null) {	
						UpdateCloudParams (true, cloudRotMat);	
					}
			}

			if (WeatherDensity){// && Application.isPlaying) { //v3.4.3
				//control coverage and wind
		...
		void UpdateCloudParams (bool Init, Material oceanMat) {
			...
			bool is_DayLight    = (SkyManager.AutoSunPosition && SkyManager.Rot_Sun_X > 0 ) | (!SkyManager.AutoSunPosition && SkyManager.Current_Time > ( 9.0f + SkyManager.Shift_dawn) 
				& SkyManager.Current_Time <= (SkyManager.NightTimeMax + SkyManager.Shift_dawn));
		...
		//	bool is_after_21  = (SkyManager.AutoSunPosition && SkyManager.Rot_Sun_X > 75) | (!SkyManager.AutoSunPosition && SkyManager.Current_Time >  (20.7f + SkyManager.Shift_dawn));
			bool is_after_222  = (SkyManager.AutoSunPosition && SkyManager.Rot_Sun_X > 85) | (!SkyManager.AutoSunPosition && SkyManager.Current_Time >  (22.0f + SkyManager.Shift_dawn));
...
//				//FoamCutoff = Foam.y;
//			} else {
//				oceanMat.SetVector ("_Foam", Vector4.Lerp (Foam, new Vector4 (0 , 0 , -0.16f, -0.21f), speedy));
//			}

			//HEAVY STORM
//			if(SkyManager.currentWeatherName == Artngame.SKYMASTER.SkyMasterManager.Volume_Weather_types.HeavyStorm){
//				//no moon-sun				
//				oceanMat.SetColor ("_BaseColor", Color.Lerp (oceanMat.GetColor ("_BaseColor"), StormCloudColor, speedy));
//	
//			}
//			//SNOW STORM
...
//				if(is_DayLight ){					
//					if(is_after_22 ){
//						oceanMat.SetColor ("_BaseColor", Color.Lerp (oceanMat.GetColor ("_BaseColor"), DuskCloudColor, speedy));
//
//					}else
//					if(is_after_17 ){
//						if( cloudType == CloudPreset.ClearDay ){
//								oceanMat.SetColor ("_BaseColor", Color.Lerp (oceanMat.GetColor ("_BaseColor"), DayCloudColor, speedy));
//						
//						}else{
//								oceanMat.SetColor ("_BaseColor", Color.Lerp (oceanMat.GetColor ("_BaseColor"), DuskCloudColor,speedy));
//							
...
//
//							}else if( cloudType == CloudPreset.ClearDay |  cloudType == CloudPreset.ClearDay){
//								oceanMat.SetColor ("_BaseColor", Color.Lerp (oceanMat.GetColor ("_BaseColor"), DayCloudColor, speedy));
//						
//						}else{
//								oceanMat.SetColor ("_BaseColor", Color.Lerp (oceanMat.GetColor ("_BaseColor"), DayCloudColor, speedy));
...
//				} else {
//					if( cloudType == CloudPreset.ClearDay ){
//						oceanMat.SetColor ("_BaseColor", Color.Lerp (oceanMat.GetColor ("_BaseColor"), DayCloudColor*0.6f, speedy));
//
//					}else{
//						oceanMat.SetColor ("_BaseColor", Color.Lerp (oceanMat.GetColor ("_BaseColor"), NightCloudColor,speedy));
//				
//					}
//				}				
//			}
			...
			if (SkyManager.windZone != null) {
				...
				float WindStr = SkyManager.windZone.windMain;

				Vector4 WindDir = new Vector4 (SkyManager.windZone.transform.forward.x, SkyManager.windZone.transform.forward.z, 0, -0.07f * 0.1f) * (-WindStr) * WindStrength * (1);
...
//					oceanMat.SetVector ("_DistortParams", Vector4.Lerp (Distortions, new Vector4 (7.82f, 0.1f, -0.74f, 6f), 0.27f * Time_delta * Sm_speed));
...
//				}
	...
	//			float _ColorDiff = oceanMat.GetFloat("_ColorDiff");
			...
			//	float _CutHeight = oceanMat.GetFloat ("_CutHeight");
				//
				//					//SCATTER
				//					_Control ("Control Color", COLOR) = (1,1,1)
				Color _Color  = oceanMat.GetColor ("_Color");
				Color _FogColor = oceanMat.GetColor ("_FogColor");
		//		float _FogFactor = oceanMat.GetFloat("_FogFactor");						
				//float _FogUnity = oceanMat.GetFloat("_FogUnity");
				if (UseUnityFog) {
				...
				float Speed_factor = 0.3f * (SkyManager.SPEED +0.1f );//0.27
				...
				if (!SkyManager.VolCloudGradients) {
					...
					if(!CurvesFromSkyMaster){
						//EvalSunIntensity = IntensitySun.Evaluate (SkyManager.calcColorTime);
						EvalLightDiff = IntensityDiff.Evaluate (SkyManager.calcColorTime) + IntensityDiffOffset + 0.30f;//v3.4.3
						EvalFog = IntensityFog.Evaluate (SkyManager.calcColorTime) + IntensityFogOffset;
					}else{
						//EvalSunIntensity = SkyManager.VcloudSunIntensity;
						EvalLightDiff = SkyManager.VcloudLightDiff  + IntensityDiffOffset + 0.30f;//v3.4.3
						EvalFog = SkyManager.VcloudFog + IntensityFogOffset;
					}
					...
					if (is_DayLight) {
						...
						if (is_after_17) { //is_after_21
							...
							if (!is_after_222) {
								if (cloudType == CloudPreset.Custom) {
									...
									oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, DuskCloudColor, 3.0f * Speed_factor * Time_delta)); //v3.4.3 - added dusk colors
						...
						//			oceanMat.SetFloat ("_FogFactor", Mathf.Lerp (_FogFactor, DuskFogFactor, Speed_factor * Time_delta));
								...
								if (cloudType == CloudPreset.Custom) {
									...
									oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, DayCloudColor * 0.3f, 0.2f * Speed_factor * Time_delta));
						...
						//			oceanMat.SetFloat ("_HorizonFactor", Mathf.Lerp (_HorizonFactor, Horizon, Speed_factor * Time_delta));
							...
							if (is_before_10) {
								if (cloudType == CloudPreset.Custom) {
									...
									oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, DawnCloudColor, 1.0f * Speed_factor * Time_delta)); //
						...
						//			oceanMat.SetFloat ("_HorizonFactor", Mathf.Lerp (_HorizonFactor, Horizon, Speed_factor * Time_delta));
							...
							//		oceanMat.SetFloat ("_FogFactor", Mathf.Lerp (_FogFactor, DawnFogFactor, Speed_factor * Time_delta));
								...
								if (cloudType == CloudPreset.Custom) {
									...
									oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, DayCloudColor, 0.3f * Speed_factor * Time_delta));
						...
						if (cloudType == CloudPreset.Custom) {
							...
							oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, NightCloudColor, 0.8f * Speed_factor * Time_delta));//v3.4.3
				...
				//v3.3d
				if (SkyManager.VolCloudGradients) {
		//			Color TMP1 = SkyManager.SkyColorGrad.Evaluate(SkyManager.Current_Time/24);
					//oceanMat.SetColor ("_FogColor", TMP1*TMP1);
		//			Color FF = 1.9f * TMP1 * TMP1* TMP1;
		//			oceanMat.SetColor ("_ShadowColor", new Color(Mathf.Clamp01(FF.r),Mathf.Clamp01(FF.g),Mathf.Clamp01(FF.b),oceanMat.GetColor ("_ShadowColor").a));
					//oceanMat.SetColor ("_Control", 0.4f*TMP1);
					if (is_DayLight) {
						oceanMat.SetVector ("sunPosition", -SkyManager.SunObj.transform.forward.normalized);
					} else {
						oceanMat.SetVector ("sunPosition", -SkyManager.MoonObj.transform.forward.normalized);
					...
					if(!CurvesFromSkyMaster){
						EvalSunIntensity = IntensitySun.Evaluate (SkyManager.calcColorTime) + IntensitySunOffset;//v3.4.3
						EvalLightDiff = IntensityDiff.Evaluate (SkyManager.calcColorTime) + IntensityDiffOffset;//v3.4.3
						EvalFog = IntensityFog.Evaluate (SkyManager.calcColorTime) + IntensityFogOffset;
					}else{
						EvalSunIntensity = SkyManager.VcloudSunIntensity + IntensitySunOffset;//v3.4.3;
						EvalLightDiff = SkyManager.VcloudLightDiff + IntensityDiffOffset;//v3.4.3
						EvalFog = SkyManager.VcloudFog + IntensityFogOffset;
					}

					if (WeatherDensity && Application.isPlaying) {
						if (SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.HeavyStorm) {
							oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, new Color (StormSunColor.r,StormSunColor.g,StormSunColor.b,EvalSunIntensity), 0.8f * Speed_factor * Time_delta + modifier));

						} else {
							oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, new Color (SkyManager.gradCloudLitColor.r, SkyManager.gradCloudLitColor.g, SkyManager.gradCloudLitColor.b, EvalSunIntensity), 0.8f * Speed_factor * Time_delta + modifier));
						}
					} else {
						oceanMat.SetColor ("_SunColor", Color.Lerp (_SunColor, new Color (SkyManager.gradCloudLitColor.r, SkyManager.gradCloudLitColor.g, SkyManager.gradCloudLitColor.b, EvalSunIntensity), 0.8f * Speed_factor * Time_delta + modifier));
					}
	//				oceanMat.SetColor ("_SunColor", Color.Lerp(_SunColor, SkyManager.gradCloudLitColor, 0.8f*Speed_factor * Time_delta+0.1f));
				//	oceanMat.SetColor ("_ShadowColor", Color.Lerp(_ShadowColor, SkyManager.gradCloudShadeColor, 0.8f*Speed_factor * Time_delta+0.1f));
					oceanMat.SetColor ("_ShadowColor", Color.Lerp(_ShadowColor, new Color(SkyManager.gradCloudShadeColor.r,SkyManager.gradCloudShadeColor.g,SkyManager.gradCloudShadeColor.b,SkyManager.VolCloudTransp), 0.8f*Speed_factor * Time_delta+modifier));
					//Debug.Log(SkyManager.calcColorTime + " "+IntensityDiff.Evaluate (SkyManager.calcColorTime));
					oceanMat.SetFloat ("_ColorDiff",EvalLightDiff);

					if (FogFromSkyGrad) {
						oceanMat.SetColor ("_FogColor", Color.Lerp (_FogColor, SkyManager.gradSkyColor, 4 * Speed_factor * Time_delta + modifier));
					} else {
						oceanMat.SetColor ("_FogColor", Color.Lerp (_FogColor, SkyManager.gradCloudFogColor, 4 * Speed_factor * Time_delta + modifier));
					}

					oceanMat.SetFloat ("_FogFactor", EvalFog);
					oceanMat.SetFloat ("_Coverage", Mathf.Lerp (_Coverage, Coverage, Speed_factor * Time_delta + modifier));
				}
				...
				if (WeatherDensity){// && Application.isPlaying) { //v3.4.3
					...
					if (SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.HeavyStorm
						...
						Coverage = Mathf.Lerp (Coverage, StormCoverage, Time_delta + 0.05f);//v3.4.3 - v3.4.5
			...
			//this_transf.localScale = Vector3.Lerp(this_transf.localScale,cloudDomeScale,0.15f *Time_delta * Sm_speed);
	...
	void Update () {
		...
		if (SkyManager != null) {			
				...
				if (cloudSidesCMat != null && SideCClouds) {	//v3.4.3
			...
			if (rainbowMat != null) {	
				...
				if (!Application.isPlaying) {
					...
					if (autoRainbow) {
						if (SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.Rain || SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.HeavyStorm
							...
							if (SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.Rain) {
								IntensityR = rainboxMaxIntensity / 1;
							} else {
								IntensityR = rainboxMaxIntensity / 3;
							}
						}
					...
					if (autoRainbow) {
						if (SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.Rain || SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.HeavyStorm
							|| SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.HeavyStormDark && !was_rain) {
							was_rain = true;
						...
						if (SkyManager.currentWeatherName == SkyMasterManager.Volume_Weather_types.Cloudy) {
							if (was_rain) {
								float DiffT = Time.fixedTime - was_rain_end_time;
									
								if (DiffT > rainbowTime) {								
									//IntensityR = 0;
									IntensityR = rainboxMaxIntensity/ DiffT;
									//rainboxMaxIntensity = Mathf.Lerp (rainboxMaxIntensity, 0, Time.deltaTime);
									if (DiffT < 0.05f || DiffT <=0) {
										was_rain = false;
										//rainboxMaxIntensity = 0;
									}
								} else {
									//IntensityR = DiffT * (rainboxMaxIntensity * 1) / rainbowTime;
									IntensityR =  rainbowTime*(rainboxMaxIntensity) / DiffT;
								}
							} else {
								IntensityR = 0;
							}
							if (IntensityR > rainboxMaxIntensity) {
								IntensityR = rainboxMaxIntensity;
							}
		...
		if (Application.isPlaying) {
				...
				if (EnableLightning) {
					...
					if (LightningBox != null) {
						if (Time.fixedTime - last_lightning_time > lightning_every - Random.Range (-lightning_rate_offset, lightning_rate_offset)) {
							...
							if (Random.Range (0, SkyManager.WeatherSeverity + 1) == 1) { 
								//do nothing
							}else{
				...
				if (rotateWithCamera && Camera.main != null) {//v3.4.3
					if (RotClouds != null && RotClouds.gameObject.activeInHierarchy) {
						//RotClouds.eulerAngles = new Vector3 (RotClouds.eulerAngles.x,Mathf.Lerp(RotClouds.eulerAngles.y,Camera.main.transform.eulerAngles.y,1.5f*Time.deltaTime),RotClouds.eulerAngles.z);
						//RotClouds.eulerAngles = new Vector3 (RotClouds.eulerAngles.x,Mathf.Lerp(RotClouds.eulerAngles.y,Camera.main.transform.eulerAngles.y,11.5f*Time.deltaTime),RotClouds.eulerAngles.z);
						...
						if (!EnableInsideClouds) {
							...
							if (LerpRot) {
								...
								//if (RotCloudsSPlane != null) {
									//RotCloudsSPlane.parent = RotClouds;
									//RotCloudsSPlane.localScale = ShadowPlaneScale;
								//}
							}
						} else {
							float Desired_rot = 0;
					...
					if (SideClouds != null && SideClouds.gameObject.activeInHierarchy) {
						if (!EnableInsideClouds) {
							...
							//v3.4.3
							//if (SideSPlane != null) {
								//SideSPlane.parent = SideClouds;
								//SideSPlane.localScale = ShadowPlaneScale;
							//}
				...
				if (followPlayer && SkyManager.Hero != null) {//v3.4.3
