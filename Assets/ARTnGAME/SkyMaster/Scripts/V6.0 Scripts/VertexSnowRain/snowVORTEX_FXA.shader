...
Shader "SkyMaster/snowRainVERTEX_SM_VORTEX_FXA" {
    Properties {
		...
		//v0.8
		planeY("planeY", Float) = 0
		hitThres("hitThres", Float) = 0.5
		hitPower("hitPower", Float) = 1		
    }
	SubShader {
        ...
        Pass {
            ...
            v2f vert(appdata_custom v)
            {
				...
				//if(0==0){
					...
					//v0.8					
					if(mv.y < planeY + hitThres && mv.y > planeY - hitThres){
						finalposition += (v.texcoord.x-0.5f)*sideVector * _Size*8;
						finalposition += (v.texcoord.y-0.5f)*diff*0.02;
						finalposition.y += hitPower;
						finalposition.x += hitPower;
						//finalposition.z -= hitPower;
					}
					if(mv.y < planeY){
						finalposition = v.vertex.xyz;
					}
