...
Shader "Custom/RaindropSM TUT STAGE3 TRANSP A MASKED" {
	...
	//			col += drop;
			...
			float4 frag(v2f i) : SV_Target{
				...
				float2 currentPoint = float2(i.uv.x + wipeControls.x, i.uv.y-0.5 + wipeControls.y);
	...
	////			i.uv.y = (i.uv.y + i.uv.y * dist);
				...
				//v0.7
				float dist3D = sqrt(
						(interactPointRadius.x - i.worldPos.x)*(interactPointRadius.x - i.worldPos.x) 
					+	(interactPointRadius.y - i.worldPos.y)*(interactPointRadius.y - i.worldPos.y)
					+	(interactPointRadius.z - i.worldPos.z)*(interactPointRadius.z - i.worldPos.z));				
				//i.uv.x = abs(i.uv.x* dist3D * dist3D*0.0005)*interactPointRadius.w;
				//i.uv.y = (i.uv.y* dist3D * dist3D*0.0005)*interactPointRadius.w;
				//i.uv.x = i.uv.x-i.uv.y*2;
				i.uv.y = 1 - i.uv.y;
				if (i.uv.x > 0.5) {

					//directionControls
					i.uv.x = i.uv.x - (i.uv.x*directionControls.y)*i.uv.y*directionControls.x ;
					if (directionControls.x == 0) {
						i.uv.x = 1 + radialControls.z * i.uv.x - radialControls.x * 2 * pow(i.uv.y, radialControls.y * 4);
					}
					else {
						...
						//directionControls
						i.uv.x = i.uv.x + i.uv.x*i.uv.y*directionControls.x;
					}
		...
		//		v2 += raindot(frac(uv * 30.0  + float2(0, 0.1 * iTime))* abs(cos(iTime*0.2)), floor(uv * 30.0 + float2(0, 0.1 * iTime)), iTime * 2);
				...
				//v *= wipeFactor;
				if (wipeFactor == 0) {
					//i.uv.x = i.uv.x*0.4*sign(wipeDirection) + (wipeDirection)*0.8 + 1.5;// +2 * i.uv.y / 3;
					i.uv.x = i.uv.x - 0.5*sign(wipeDirection)* i.uv.y;// +2 * i.uv.y / 3;
				}
		...
		//		projUV += 0.05*(float2(ddx(v*0.4), ddy(v * 0.4)) + float2(ddx(v2 * 0.4), ddy(v2 * 0.4))) * drops.w;// n * dropPosition;
				...
				for (float j = 0; j < numSamples; j++) {
					...
					///col += tex2D(_GrabTexture, projUV * eraser);
