...
//#define WORLDAPI_PRESENT
    ...
    /// 
    /// </summary>
    public sealed class WorldManagerSkyMasterWAPI : MonoBehaviour, IWorldApiChangeHandler
    {



    #region Interfaces
        public int sampleWeather = 0;//0=no weather, 1=rain, 2=snow, 3=clouds
        public float timeSpeed = 1;
        public float startTime = 9;
        public float cloudTransitionSpeed = 5;

        void Start()
        {
            //v0.1
            SetDecimalTime(startTime);
    ...
    #region Unity Update and LateUpdate methods
        ...
        public bool revertToSunnyWeather = true;
        ...
        void LateUpdate()
        {
            ...
            float m_timeNow = -0.000001f;
            if (sampleWeather > 0)// Start timer if use sampless
            {
                ////v0.1 - TIME
                m_timeNow = (float)GetTimeDecimal() ;
                m_timeNow += 0.25f * 0.01f * timeSpeed;
                //WorldManager.Instance.SetDecimalTime(m_timeNow);
                SetDecimalTime(m_timeNow);
                //skyManager.Current_Time = m_timeNow;

                //reset all
                Rain = new Vector4(0, 0, 0, 0);
                ThunderPower = 0;
                Snow = new Vector4(0, 0, 0, 0);
                Clouds = new Vector4(-1, 0, 0, 0);
            }
            ...
            if (sampleWeather == 1)
            {
                ...
                // Get and set rain -x = RainPower, y = RainPowerOnTerrain, z = RainMinHeight, w = RainMaxHeight
                /// </summary>
                Rain = new Vector4(1, 0, 1, 2);
            }
            if (sampleWeather == 2)
            {               
                Snow = new Vector4(1, 0, 1, 2);
            }
            if (sampleWeather == 3)
            {               
                ThunderPower = 1;
                ...
                if (m_changeMask != 0)
                {
                    ...
                    if ((m_changeMask & WorldConstants.WorldChangeEvents.GameTimeChanged) != 0)
                    {
                        ...
                        if (m_timeNow >= 0)
                        {
                            ...
                            skyManager.Current_Time = m_timeNow;// (float)GetTimeDecimal();
                        }

                        //float timed = changeArgs.manager.GetTimeDecimal(); 
                    ...
                    if ((m_changeMask & WorldConstants.WorldChangeEvents.RainChanged) != 0)
                    {
                        ...
                        if (Rain.x > 0)
                        {
                            ...
                            skyManager.WeatherSeverity = Rain.x * 25;
                        }
                        else
                        {
                        ...
                        //set clouds to rain ones
                        skyManager.volumeClouds.useWeather = true;
                    ...
                    if ((m_changeMask & WorldConstants.WorldChangeEvents.SnowChanged) != 0)
                    {
                        ...
                        if (Snow.x > 0)
                        {
                            ...
                            skyManager.WeatherSeverity = Snow.x * 25;
                        }
                        else
                        {
                            //skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.Cloudy;
                    ...
                    if ((m_changeMask & WorldConstants.WorldChangeEvents.CloudsChanged) != 0)
                    {
                        ...
                        {
                            skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.Cloudy;
                            skyManager.volumeClouds.weatherTransitionSpeed = cloudTransitionSpeed;
                        }
                        else
                        {
    ...
    #region Event management
        ...
        public void OnWorldChanged(WorldChangeArgs changeArgs)
        {
            ...
            {
                //Grab game time
                //float m_timeNow = (float)changeArgs.manager.GetTimeDecimal();

                //Do whatever logic you want
                //m_timeNow += 0.25f;

                //v0.1
                float TOD = (float)changeArgs.manager.GetTimeDecimal();
                if (TOD != skyManager.Current_Time)
                {
                    skyManager.Current_Time = TOD;// (float)GetTimeDecimal();
                }

                //v0.2
                if(revertToSunnyWeather && changeArgs.manager.Rain.x == 0 && changeArgs.manager.Snow.x == 0 && changeArgs.manager.ThunderPower == 0)
                {
                    skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.Sunny;
                }


                if (changeArgs.HasChanged(WorldConstants.WorldChangeEvents.RainChanged))
                //   if ((WorldConstants.WorldChangeEvents.RainChanged) != 0)
                {
                    //Shader.SetGlobalVector("_WAPI_Rain", m_rainData);
                    if (debugMode)
                    {
                        Debug.Log("Rain changed: " + changeArgs.manager.Rain.x);
                    }
                    //v0.1
                    if ((float)changeArgs.manager.Rain.x > 0)
                    {
                        if (debugMode)
                        {
                            Debug.Log("Setting rain on Sky Master");
                        }
                        skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.Rain;
                        skyManager.WeatherSeverity = changeArgs.manager.Rain.x * 20;
                    }
                    else
                    {
                        //skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.Cloudy;
                ...
                if ((WorldConstants.WorldChangeEvents.HailChanged) != 0)
                {
                    Shader.SetGlobalVector("_WAPI_Hail", m_haildata);
                }

                if (changeArgs.HasChanged(WorldConstants.WorldChangeEvents.SnowChanged))
                //    if (( WorldConstants.WorldChangeEvents.SnowChanged) != 0)
                {
                    Shader.SetGlobalVector("_WAPI_Snow", m_snowData);
                    if (debugMode)
                    {
                        Debug.Log("Snow changed: " + changeArgs.manager.Snow.x);
                    }
                //v0.1
                if ((float)changeArgs.manager.Snow.x > 0)
                {
                        if (debugMode)
                        {
                            Debug.Log("Setting snow on Sky Master");
                        }
                        skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.SnowStorm;
                        skyManager.WeatherSeverity = changeArgs.manager.Snow.x * 20;
                    }
                    else
                    {
                        //skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.Cloudy;
                ...
                if (changeArgs.HasChanged(WorldConstants.WorldChangeEvents.ThunderChanged))
                //    if (( WorldConstants.WorldChangeEvents.ThunderChanged) != 0)
                {
                    Shader.SetGlobalVector("_WAPI_Thunder", m_thunderData);
                    if (debugMode)
                    {
                        Debug.Log("Thunder changed: " + changeArgs.manager.ThunderPower);
                    }
                    if ((float)changeArgs.manager.ThunderPower > 0)
                    {
                        skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.LightningStorm;
                    }
                    else
                ...
                if (changeArgs.HasChanged(WorldConstants.WorldChangeEvents.CloudsChanged))
                //    if (( WorldConstants.WorldChangeEvents.CloudsChanged) != 0)
                {
                    Shader.SetGlobalVector("_WAPI_Clouds", m_cloudData);
                    if (debugMode)
                    {
                        Debug.Log("Clouds changed: " + changeArgs.manager.Clouds.x);
                        Debug.Log("Clouds with rain: " + changeArgs.manager.Rain.x);
                        Debug.Log("Clouds with snow: " + changeArgs.manager.Snow.x);
                        Debug.Log("Clouds with thunder: " + changeArgs.manager.ThunderPower);
                    }
                    //v0.1
                    if ((float)changeArgs.manager.Clouds.x > 0)
                    {                        
                        if ((float)changeArgs.manager.Snow.x > 0)
                        {
                            if (debugMode)
                            {
                                Debug.Log("Setting snow clouds on Sky Master");
                            }
                            skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.SnowStorm;
                            skyManager.WeatherSeverity = changeArgs.manager.Snow.x * 20;
                            skyManager.volumeClouds.weatherTransitionSpeed = cloudTransitionSpeed;
                        }
                        else if ((float)changeArgs.manager.Rain.x > 0  || (float)changeArgs.manager.ThunderPower > 0)
                        {                             
                            if ((float)changeArgs.manager.ThunderPower > 0)
                            {
                                if (debugMode)
                                {
                                    Debug.Log("Setting lightning clouds on Sky Master");
                                }
                                skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.LightningStorm;
                                skyManager.volumeClouds.weatherTransitionSpeed = cloudTransitionSpeed;
                                skyManager.volumeClouds.lightning_rate_offset = (float)changeArgs.manager.ThunderPower * 2;
                            }
                            else
                            {
                                if (debugMode)
                                {
                                    Debug.Log("Setting rain clouds on Sky Master");
                                }
                                skyManager.currentWeatherName = SkyMasterManager.Volume_Weather_types.Rain;
                                skyManager.WeatherSeverity = changeArgs.manager.Rain.x * 20;
                                skyManager.volumeClouds.weatherTransitionSpeed = cloudTransitionSpeed;
    ...
    #region Singleton & general class management
        ...
        private WorldManagerSkyMasterWAPI() {}

        //This is a singleton - here is the instance
        private static WorldManagerSkyMasterWAPI m_instance;

        //Access to the world manager instance
        public static WorldManagerSkyMasterWAPI Instance
        {
            get
            {
                if (m_instance == null)
                {
                    m_instance = (WorldManagerSkyMasterWAPI)FindObjectOfType(typeof(WorldManagerSkyMasterWAPI));
                    if (m_instance == null)
                    {
                        GameObject singleton = new GameObject();
                        singleton.hideFlags = HideFlags.HideAndDontSave;
                        singleton.name = typeof(WorldManagerSkyMasterWAPI).ToString();
                        m_instance = singleton.AddComponent<WorldManagerSkyMasterWAPI>();
                    }
                }
                return m_instance;
            }
    ...
    public class WorldManagerSkyMasterWAPI : MonoBehaviour
    {
