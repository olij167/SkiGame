...
Shader "SkyMaster/Water4 SM3.0 Local Lights - Waves - iOS" {
Properties {
...
CGINCLUDE
	...
	half4 frag600( v2f i ) : SV_Target
	{
		...
		half refrFix = SAMPLE_DEPTH_TEXTURE_PROJ(_camDepthTex, UNITY_PROJ_COORD(grabWithOffset)); //_camDepthTex  -- _CameraDepthTexture - iOS
		...
		#ifdef WATER_EDGEBLEND_ON
			half depth = SAMPLE_DEPTH_TEXTURE_PROJ(_camDepthTex, UNITY_PROJ_COORD(i.screenPos)); //_camDepthTex  -- _CameraDepthTexture - iOS
	...
	v2f vert(appdata_full v)
	{
		...
		v.vertex.xyz += float3(offsets.x, offsets.y *(pow(tex.r+0.00000001,_ShoreFadeFactor)),offsets.z);
...
//		}
		...
		if ((1 - i.screenPos.z) * 50 > (fogThres*0.05 + 105)*0.3) {
			baseColor = lerp(baseColor, fogColor, clamp(0.001*fogBias*pow((1 - i.screenPos.z) * 50 - (fogThres*0.05 + 105)*0.3, fogDepth * 9 * 0.8), 0, 1));
		}
		#endif
