...
namespace Artngame.SKYMASTER
{
    public class lightningCameraVolumeCloudsSM_SRP : MonoBehaviour
    {
        ...
        public CloudScript fullvolumeCloudsScript;//public connectSuntoFullVolumeCloudsURP fullvolumeCloudsScript;
        ...
        public void createLightningBox()
        {
            if (setupLightning)
            {
                if (LightningBox == null)
                {
                    ...
                    {
                        GameObject goA = Resources.Load("Volume Clouds Lightning Prefab") as GameObject;
                        LightningPrefab = goA;

                        GameObject go = Resources.Load("Volume Clouds Lightning Bounding Box") as GameObject;
                        Debug.Log(go.name);
                        LightningBoxPrefab = go;
                        LightningBox = go.transform;
                    }
                    else {
                ...
                if (EnableLightning)
                {
                    ...
                    if (LightningBox != null)
                    {
                        if (Time.fixedTime - last_lightning_time > lightning_every - UnityEngine.Random.Range(-lightning_rate_offset, lightning_rate_offset))
                        {
                            ...
                            if (SkyManager != null && UnityEngine.Random.Range(0, SkyManager.WeatherSeverity + 1) == 1)
                            {
                                //do nothing
                            }
                            else
