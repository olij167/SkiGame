...
// MIT LICENSE https://github.com/ya7gisa0/Unity3D-Lightning-Shader
...
#define LIGHTNING_PARTS 10
	...
	float lightning2(float2 ls, float2 le, float intensity, float2 uv)
	{
		...
		[unroll(10)]for (int i = 0; i < LIGHTNING_PARTS; i++)
		{
			...
			float lh1 = lhash(ltemp1.y, _Time.y/2);
			float lh2 = lhash(ltemp2.y, _Time.y/2);
			ltemp1.x += lh1;
			ltemp2.x += lh2;
	...
	float lightning(float2 ls, float2 le, float intensity, float2 uv)
	{
		...
		for (int i = 0; i < LIGHTNING_PARTS; i++)
		{
			...
			float lh1 = lhash(ltemp1.y + ltemp1.x, _Time.y/2);
			float lh2 = lhash(ltemp2.y + ltemp2.x, _Time.y / 2);
			ltemp1.x += lh1;
			ltemp2.x += lh2;
	...
	fixed4 frag(v2f i) : SV_Target
	{
		...
		{
			//Lightning Seeds
			float h = hash(float2(sqrt(float(i)), floor((_Time.y / 2 + _CustomTime) * 7.0 + (float(i) / 10.0)) * float(i)));
			//ls.x = hash(h) * 3.0 - fmod(_Time.y, 3.0);
			// place the lightning in the center area
			ls.x = hash(h) * 0.1 - fmod(_Time.y / 2 + _CustomTime, 0.1);
