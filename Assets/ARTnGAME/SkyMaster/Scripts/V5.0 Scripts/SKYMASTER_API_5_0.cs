...
namespace Artngame.SKYMASTER {
	public class SKYMASTER_API_5_0 : MonoBehaviour {
        ...
        //v5.0
        bool volumeClouds1 = true;
        bool volumeClouds2 = false; //fly through clouds
        public CloudScript volumeClouds2SCRIPT;
        public WeatherScript WeatherPatternSCRIPT;

        public CloudScript volumeClouds2SCRIPT_REFL; // Cloud script for reflection camera, fly through clouds
...
//			}
        ...
        void OnGUI() {
			...
			if(HUD_ON){	
				...
				SUNMASTER.VolShaderCloudsH.IntensityDiffOffset = GUI.HorizontalSlider(new Rect (7 * BOX_WIDTH + 35+10, BOX_HEIGHT + 60 + 30 +0+60-5-3, BOX_WIDTH+10, 30),SUNMASTER.VolShaderCloudsH.IntensityDiffOffset,-0.35f,0.35f);
				...
				SUNMASTER.VolShaderCloudsH.ClearDayHorizon = GUI.HorizontalSlider(new Rect (5 * BOX_WIDTH + 20, BOX_HEIGHT + 60 + 0 +0+60-5-13, BOX_WIDTH, 30),SUNMASTER.VolShaderCloudsH.ClearDayHorizon,0,0.4f);
...
//					}
			...
			if (GUI.Button (new Rect (2, 5 * BOX_HEIGHT, BOX_WIDTH - 2, 22), "Toggle sky")) {	
                ...
                { //v5.0 - if old clouds chosen, control the coverage per type
                    ...
                    {
                        SUNMASTER.VolShaderCloudsH.ClearDayCoverage = GUI.HorizontalSlider(new Rect(6 * (BOX_WIDTH + XOffset), 3 * BOX_HEIGHT + 25 + 10, BOX_WIDTH * 4 + 15, 30), SUNMASTER.VolShaderCloudsH.ClearDayCoverage, -0.55f, -0.05f);
                    }
                    ...
                    {
                        SUNMASTER.VolShaderCloudsH.ClearDayCoverage = GUI.HorizontalSlider(new Rect(6 * (BOX_WIDTH + XOffset), 3 * BOX_HEIGHT + 25 + 10, BOX_WIDTH * 4 + 15, 30), SUNMASTER.VolShaderCloudsH.ClearDayCoverage, -1.25f, 1.05f);
                    }
                    ...
                    if (CloudHandler.MultiQuadCClouds)
                    { //v3.4.3
                        SUNMASTER.VolShaderCloudsH.ClearDayCoverage = GUI.HorizontalSlider(new Rect(6 * (BOX_WIDTH + XOffset), 3 * BOX_HEIGHT + 25 + 10, BOX_WIDTH * 4 + 15, 30), SUNMASTER.VolShaderCloudsH.ClearDayCoverage, -15.55f, 5f);
                    }
                }
                else {
                    ...
                    {
                        //Grab the voluem clouds reference from the sky master manager script
                        volumeClouds2SCRIPT.coverage = GUI.HorizontalSlider(new Rect(6 * (BOX_WIDTH + XOffset), 3 * BOX_HEIGHT + 25 + 10, BOX_WIDTH * 4 + 15, 30), volumeClouds2SCRIPT.coverage, 0f, 1.6f);
                ...
                if (GUI.Button(new Rect(3 * BOX_WIDTH + 10, BOX_HEIGHT + 60 + 30 + 31 + 31 + 31, BOX_WIDTH * 2, 32), "Toggle Fly Through Clouds"))
                {
                    volumeClouds1 = false; //Disable image effect volume clouds
                    SUNMASTER.volumeClouds.enabled = false;
                    SUNMASTER.volumeClouds.reflectClouds.enabled = false;
