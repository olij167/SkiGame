...
namespace Artngame.SKYMASTER {
    ...
    public class SkyMasterManager : MonoBehaviour
    {
        ...
        [Range(-3.5f, 1)] //[Range(-1.5f, 0)] //v5.1.5k
...
//						skyboxMat.SetFloat("White_cutoff",  Mathf.Lerp(skyboxMat.GetFloat("White_cutoff"),0.99f,7*SpeedA*FinalSPEED));
			...
			if (UseGradients || VolCloudGradients) { //v3.4.3
				...
				if (UseGradients && SkyColorGrad != null) {
					...
					m_TintColor = SkyTintGrad.Evaluate(time_point) + new Color(tintColorOffset.x, tintColorOffset.y, tintColorOffset.z, 0);
					...
					if (FexposureC != null) {
						m_fExposure = FexposureC.Evaluate (time_point) + 0.4f + fExposureOffset;
                    }
					if (FscaleDiffC != null) {
						scale_dif = FscaleDiffC.Evaluate (time_point) + scaleDifOffset;
					}

					if (FSunGC != null) {
						m_g = -0.1f * FSunGC.Evaluate (time_point) - 0.9f + gOffset;
					}
					if (FSunringC != null) {
						Sun_ring_factor = FSunringC.Evaluate (time_point) + sunRingFactorOffset;
					}
...
//					}
					...
					if (currentWeatherName == Volume_Weather_types.HeavyStorm || currentWeatherName == Volume_Weather_types.HeavyStormDark) {						
						...
						m_fWaveLength = new Vector3 (TMP1.r, TMP1.g, TMP1.b) + fWaveLengthOffset;
