...
Shader "SkyMaster/SkyMasterShaderE_SKYBOX U5 No Ground" 
{ 
 ...
 Properties {
		...
		_Obliqueness("Obliqueness", Vector) = (0, 0, 0, 0)
			...
			adjustGround("adjust Ground", Float) = 0
				adjustGround2("adjust Ground", Float) =1
 }
	 ...
	 SubShader
		...
		float adjustGround2;
			...
			fragIO OUTPUT;
				...
				float3 v3Start =((v3Ray - 0) * (0+10.1))+ ( - v3CameraPos) -float3(0,-Horizon_adj,0) ;
...
//			half3 sun = (IN.normalAndSunExp.w > 0) ? pow(max(0.0, dot(normal, sunDir)), IN.normalAndSunExp.w) : 0.0;
      ...
      float3 NEW_D = normalize(IN.v3Direction);
                ...
                float4 Out_Color = (float4(Scolor,1.0)+(_Coloration*result))+_TintColor;  
                  ...
                  if(IN.normalAndSunExp.y < HorizonY-adjustGround/100){
                  	Out_Color = GroundColor + Out_Color*(IN.normalAndSunExp.y - (HorizonY - adjustGround / 100)) ;
                  }
                    
                        
                 return saturate(Out_Color);//+2*_Coloration*result1;                
