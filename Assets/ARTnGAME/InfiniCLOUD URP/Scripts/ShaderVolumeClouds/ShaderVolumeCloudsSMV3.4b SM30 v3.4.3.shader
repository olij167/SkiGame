...
Shader "SkyMaster/ShaderVolumeClouds-Desktop-SM3.0 v3.4.3" {
    Properties {
        ...
        _TranspOffset ("_Transparency Offset", Float ) = 0
            ...
            _shadowCoverageOffset("_Shadow Coverage Offset", Float) = 0
            _shadowBias("_Shadow Bias", Float) = 0
            _shadowToggleRadius("_shadow Toggle Radius", Float) = 0
...
//	#pragma multi_compile_fwdbase nolightmap //v4.1
    ...
    float _shadowCoverageOffset;
float _shadowBias;
float _shadowToggleRadius;
            ...
            float4 GetShadowCasterPositionCS(float3 positionWS, float3 normalWS) {
                float3 lightDirectionWS = _LightDirection;// _WorldSpaceLightPos0.xyz;// _LightDirection;

                if (length(_WorldSpaceCameraPos.xz - positionWS.xz) < _shadowToggleRadius) {
                    _shadowBias = 19000;
                }

                float4 positionCS = TransformWorldToHClip(ApplyShadowBias(positionWS, normalWS* _shadowBias, lightDirectionWS));
//#if UNITY_REVERSED_Z
...
//#endif
...
#endif
                //https://github.com/Unity-Technologies/Graphics/blob/f4b33bfe8ce98bd1ade9d8e05792eb1cde2d6e4e/Packages/com.unity.render-pipelines.universal/ShaderLibrary/Shadows.hlsl#L545
            ...
            Interpolators Vertex(Attributes input) {
                ...
                output.worldPos = posnInputs.positionWS;//mul(unity_ObjectToWorld, input.positionOS);
            ...
            float4 fragA(VertexOutput i) : COLOR {*/
               ...
               // float4 paintTexture1 = tex2D(_PaintMap,UVs1+_CloudMap1_ST);
                ...
                float DER = i.worldPos.y*0.001;               
                float3 normalA = (((DER*((_Coverage+_CoverageOffset+ _shadowCoverageOffset)+((cloudTexture.rgb*2)-1)))-(1-(Texture1.rgb*2))));
             	float3 normalN = normalize(normalA); 
...
//             	}
      ...
      //       	float shaper = (_Transparency-0.48+0.0)*((DER1*saturate((_Coverage+cloudTexture1.a)))-Texture2.a);		////////////////////////////////////////
                ...
                float shaper = (_Transparency + 4.5 + _TranspOffset) * ((DER1 * saturate(((_Coverage + _CoverageOffset+ _shadowCoverageOffset) 
                    - (0.8 * PosDiff) + cloudTexture1.a * (Texture2.a)))));
