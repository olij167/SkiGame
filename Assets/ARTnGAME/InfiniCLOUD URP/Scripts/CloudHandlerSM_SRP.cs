...
public class CloudHandlerSM_SRP : MonoBehaviour {
	...
	//	Color DuskCloudColor = new Color(191f/255f,99f/255f,119f/255f,209f/255f);
		...
		public bool setStartWind = false;//v3.5
		...
		void UpdateCloudParamsStart(Material oceanMat)
        {
			float WindStr = windZone.windMain;
			Vector4 WindDir = new Vector4(windZone.transform.forward.x, windZone.transform.forward.z, 0, -0.07f * 0.1f) * (-WindStr) * WindStrength;
			oceanMat.SetVector("_Velocity1", WindDir);
			oceanMat.SetVector("_Velocity2", new Vector4(WindDir.x * WindParallaxFactor,
				WindDir.y * WindParallaxFactor, 0.0f, CloudEvolution));
		}
		...
		if (Application.isPlaying) {
				...
				//v3.5
				//APPLY WIND
				if (setStartWind && windZone != null)
				{	
					if (cloudFlatMat != null && DomeClouds)
					{
						UpdateCloudParamsStart(cloudFlatMat);
					}
					if (cloudSidesMat != null && SideClouds)
					{
						UpdateCloudParamsStart( cloudSidesMat);
					}
					if (cloudSidesAMat != null && SideAClouds)
					{
						UpdateCloudParamsStart( cloudSidesAMat);
					}
					if (cloudSidesBMat != null && SideBClouds)
					{
						UpdateCloudParamsStart( cloudSidesBMat);
					}
					if (cloudSidesCMat != null && SideCClouds)
					{   //v3.4.3
						UpdateCloudParamsStart( cloudSidesCMat);
					}
					if (cloudRotMat != null && OneQuadClouds)
					{
						UpdateCloudParamsStart( cloudRotMat);
					}					
				}
		...
		void UpdateCloudParams (bool Init, Material oceanMat) {
            ...
            if (oceanMat.HasProperty("vortexControl"))
            {
                if (grabVortexParamsOfShader)
                {
                    ...
                    Vector4 paramsVortex = oceanMat.GetVector("vortexControl");
                    Vector4 posVortex = oceanMat.GetVector("vortexPosRadius");
                ...
                oceanMat.SetVector("vortexControl", new Vector4(vortexVorticity, vortexSpeed, vortexThickness, vortexCutoff));
                oceanMat.SetVector("vortexPosRadius", new Vector4(vortexPosition.x, vortexPosition.y, vortexPosition.z, 1));
            }
	...
	//			float _ColorDiff = oceanMat.GetFloat("_ColorDiff");
			...
			//	Vector4 _LightingControl = oceanMat.GetVector ("_LightingControl");   
                ...
                if(lightningScript1 != null && !LightningOne.gameObject.activeInHierarchy && ((lightningScript2 != null && !LightningTwo.gameObject.activeInHierarchy) || lightningScript2 == null))
                {
	...
	public void Update () { //v1.7
		...
		if (Application.isPlaying) {
				...
				if (EnableLightning) {
					...
					if (LightningTwo == null && useSecondLightning) {
