    ...
    public static class ProceduralTextureGenerator
    {
        ...
        private static void RasterizeSites(ProceduralTextureGenerationSettings settings, List<ProceduralTextureSite> sites, float[] values)
        {
            ...
            for (int i = 0; i < sites.Count; i++)
            {
                ...
                int minX = Mathf.Clamp(Mathf.FloorToInt(site.Position01.x * (width - 1) - radiusPixels - 1), 0, width - 1);
                int maxX = Mathf.Clamp(Mathf.CeilToInt(site.Position01.x * (width - 1) + radiusPixels + 1), 0, width - 1);
                int minY = Mathf.Clamp(Mathf.FloorToInt(site.Position01.y * (height - 1) - radiusPixels - 1), 0, height - 1);
                int maxY = Mathf.Clamp(Mathf.CeilToInt(site.Position01.y * (height - 1) + radiusPixels + 1), 0, height - 1);
                ...
                for (int y = minY; y <= maxY; y++)
                {
                    ...
                    for (int x = minX; x <= maxX; x++)
                    {
                        ...
                        Vector2 local = new Vector2(px - site.Position01.x, py - site.Position01.y);
                        float distance01 = local.magnitude / Mathf.Max(Epsilon, site.Radius01);
                        if (distance01 > 1f)
                            continue;
        ...
        private static float SampleStampTexture(ProceduralTextureGenerationSettings settings, Vector2 local01, float radius01, float cos, float sin)
        {
            ...
            try
            {
                c = source.GetPixelBilinear(Mathf.Clamp01(u), Mathf.Clamp01(v));
            }
        ...
        private static void GenerateRandomMinSpacing(ProceduralTextureGenerationSettings settings, System.Random rng, List<ProceduralTextureSite> results)
        {
            ...
            while (results.Count < settings.count && tries-- > 0)
            {
                ...
                if (!HasMinSpacing(position, settings.minSpacing01, results))
                    continue;
            ...
            bool PoissonValid(Vector2 candidate)
            {
                ...
                for (int y = -2; y <= 2; y++)
                {
                    for (int x = -2; x <= 2; x++)
                    {
                        ...
                        if (sampleIndex >= 0 && Vector2.Distance(candidate, samples[sampleIndex]) < minRadius)
                            return false;
                    }
                }
                return true;
        ...
        private static bool HasMinSpacing(Vector2 position, float minSpacing, List<ProceduralTextureSite> sites)
        {
            if (minSpacing <= 0f)
                return true;
            ...
            for (int i = 0; i < sites.Count; i++)
            {
                if ((sites[i].Position01 - position).sqrMagnitude < minSq)
                    return false;
            }
            return true;
        }
        ...
        private static float SampleDensity(ProceduralTextureGenerationSettings settings, Vector2 position01)
        {
            ...
            float u = Mathf.Repeat(position01.x * settings.densityTiling.x + settings.densityOffset.x, 1f);
            float v = Mathf.Repeat(position01.y * settings.densityTiling.y + settings.densityOffset.y, 1f);
            try
            {
