...
namespace PungentFunk.Utilities.Editor.PreviewExport
{
    ...
    {
        private const string GenericMenuPath = "Tools/Utilities/Prefabs/Save Selected As Prefab With Mesh Assets";

        [MenuItem(GenericMenuPath, priority = 1000)]
        private static void SaveSelectedAsPrefabWithMeshAssets()
        {
            SaveSelectedWithOptions(new ExportOptions());
        }

        [MenuItem(GenericMenuPath, true)]
        private static bool ValidateSaveSelectedAsPrefabWithMeshAssets()
        {
